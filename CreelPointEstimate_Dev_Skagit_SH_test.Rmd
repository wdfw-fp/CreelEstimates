...
params:
  ...
  date_end: "2021-04-13"
  time_strata: "month"
  sections: "lut_water_body_location_d14_skagit_winter_sthd.csv"
output:
  html_document:
    fig_caption: yes
    theme: default
...
lut <- map(
  list(
    ...
    census_expansion = "input_files/lut_Proportional_Expansions_for_Tie_In_Sections_Skagit_Steelhead_2021_Example.csv"
  ),
...
# 2021 Skagit Winter Steelhead C&R fishery only open 4 days / week
...
# EB 3/2/2022 interview question "fish from boat or bank?" not yet used in this project, so need to classify angler type from interviews with different criteria 
...
point_estimate_data_prelim$interview <- point_estimate_data_prelim$interview |> 
  ...
  mutate(
    ...
    angler_type = case_when(
      is.na(boat_type) ~ "bank",
      boat_type == "Pontoon/Kick/Kayak/Dinghy" ~ "bank",
      boat_type %in% c("Drift Boat/ Raft", "Trailered Motorized Boat") ~ "boat"
      ),
...
estimates$daily_effort_cpue_catch <- point_estimate_data_prelim$effort_interviews_final |> 
  ...
  mutate_if(is.numeric, round, 2) |> 
    filter(str_detect(catch_group, 'Steelhead|Bull')) |>  # placeholder filter 

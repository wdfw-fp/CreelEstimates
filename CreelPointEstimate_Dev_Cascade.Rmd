...
estimates$time_strata_catch_total |> 
  # filter(!grepl(c("Steelhead"),Catch_Group)) |> 
  filter(catch_group %in% c("Chinook_Adult_AD_Kept", "Chinook_Adult_UM_Released")) |>
  ggplot(aes(x = time_strata, y = total_catch)) +
...
#   theme(legend.text=element_text(size=8)) +
#   theme_bw(base_size = 12) + 
#   facet_wrap(. ~ section + catch_group)
...
estimates$cpue_time_strata <- estimates$time_strata_catch_total  |>
  ...
  left_join(effort_rename) |> 
  mutate(
    cpue_time_strata = total_catch / total_effort,
...
estimates$cpue_time_strata |> 
  filter(catch_group %in% c("Chinook_Adult_AD_Kept", "Chinook_Adult_AD_Relased","Chinook_Adult_UM_Released")) |> 
  ggplot(aes(x = time_strata, y = cpue_time_strata)) +
  geom_point(position = "dodge", stat = "identity", color = "black") +
  # scale_fill_manual(values=c("#999999", "#E69F00")) +

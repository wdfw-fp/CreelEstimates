...
if(str_detect(params$index_count_types, "Bank|Boat")) {
  ...
  pe_estimates$interview_ang_per_vehic <- bind_rows(
    pe_data$interview |>
      ...
      summarize(angler_final = "total", anglers_per_vhcl_trlr = sum(angler_count) / sum(vehicle_count), .groups = "drop")
    ,
    pe_data$interview |>
      filter(angler_final == "boat") |>
      #could re-introduce here: group_by(section_num)
      summarize(angler_final = "boat", anglers_per_vhcl_trlr = sum(angler_count) / sum(trailer_count), .groups = "drop")
    )

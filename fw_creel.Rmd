...
date: "`r Sys.Date()`"
params:
  fishery_name: "Skykomish summer Chinook 2022"
...
#  dplyr::select(water_body_desc, location = location_code, section, section_name)
# 
...
#dwg$effort |> count(location_type, survey_type, tie_in_indicator, location_season_name, section_num)

#!DA: hopefully this mostly goes away when lat/lon added to view(s)
ll <- readr::read_csv("input_files/river_locations_master.csv") |> 
  dplyr::filter(River %in% dwg$effort$water_body[1]) |> #input_analysis$water_body 
  dplyr::distinct(River, .keep_all = T) |> 
  dplyr::select(Lat,Long)

days <- prep_days(
  date_begin = head(lu_input$closures$event_date, 1),
  date_end = tail(lu_input$closures$event_date, 1),
  weekends = params$days_wkend,
  holidays = read_lines("input_files/dates_holidays_2015_2030.txt") |> as.Date(format="%Y-%m-%d"), #lu_input$dates_holidays_2015_2030
  Lat = ll$Lat, Long = ll$Long,
  mod_per = params$model_period,
  closures = lu_input$closures
  )
...
```{r}
# display the section names so user can crosswalk section number to section name in subsequent output

pe$section_table <- lu_input$sections |>
...
pe$effort_census <- prep_effort_census(eff = dwg$effort)

#Aggregates index effort counts over locations within count_seq & section
#!DA 9/22 pause pending resolution of records without fishery_name that were previously returned
pe$effort_index #<- prep_effort_index(eff = dwg$effort, days = days)

creel$effort |> filter(is.na(fishery_name))
anti_join(creel$effort, dwg$effort, by = "effort_event_id") |> arrange(event_date)
...
pe$interview <- dplyr::left_join(
  ...
  dplyr::mutate(
    ...
    angler_hours_total = total_group_count * angler_hours # changed this to total group count to match census count protocol
  ) |>

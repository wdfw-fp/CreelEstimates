...
#!! TEMPORARY hopefully this mostly goes away when lat/lon added to view(s)
...
days <- prep_days(
  ...
  closures = read_csv("input_files/closures.csv", show_col_types = FALSE) |> 
    filter(fishery_name == params$fishery_name) #coming from DWG eventually..
  )
...
dwg$effort |> 
  ...
  arrange(section_num) |>
  select(Section = section_num, Description = location) |> 
  gt()
```
...
#get count_type levels from interview to ensure alignmnet...?

#prep_interview() excludes observations with NA vehicle_count/trailer_count
#for fisheries where these were not collected, the raw data require adjustment before calling
#e.g., int_adg <- dwg$interview |> mutate(dplyr::across(c(vehicle_count, trailer_count), ~replace_na(., 0)))
...
dwg_summ$effort_census <- prep_effort_census(eff = dwg$effort)

dwg_summ$effort_index <- prep_effort_index(eff = dwg$effort)

#MOVE angler_final MUTATE in current function TO LATER IN SCRIPT TO ALLOW SUMMARIZATION SHARED WITH BSS
...
#START WITH FUNCTIONALIZING STAN_DAT?

...
inputs_pe$census_expan <- dwg$effort |> 
  filter(location_type == "Section") |> 
  distinct(section_num, p_census_bank, p_census_boat) |>
  pivot_longer(
...
inputs_pe$effort_index<-   
  dwg_summ$effort_index |>
  left_join(dwg$days |> select(event_date, DayType, period), by=c("event_date")) 

inputs_pe$effort_census<-
...
if(str_detect(params$index_count_types, "Bank|Boat")) {
  ...
  pe_estimates$angler_hours_daily_mean <- full_join(
    ...
    inputs_pe$effort_index |> #already in total/boat
...
pe_estimates$df <- left_join(
  ...
  dwg$days |> select(event_date, period),
  by = "event_date"
) |> 
...
pe_estimates$est_effort_s_ts_dt_at <- left_join(
  ...
  right_join(inputs_pe$days_total, by = c("section_num", "period", "DayType")) |> 

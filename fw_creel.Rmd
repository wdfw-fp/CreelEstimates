...
```{r prep_inputs_bss_single_est_cg}
#placeholder pending choices about multiple BSS est_cg per script
ecg <- unique(dwg_summ$interview$est_cg)[2]
...
```{r estimate_bss_single_est_cg}
#need to revisit to agree on approach to iterating multiple est_cg
#this presumes an inputs_bss for a single est_cg
#though note that est_cg can incorporate multiple fate, mark_status, stage, etc.
estimates_bss <- fit_bss(
  bss_inputs_list = inputs_bss,
  n_iter = 200
...
```{r prep_inputs_bss_multiple_est_cg, eval=FALSE}
inputs_bss_mult <- unique(dwg_summ$interview$est_cg)[2:3] |> 
  set_names() |> 
  map(
    ~prep_inputs_bss(
      est_catch_group = .x,
...
#identical(inputs_bss, inputs_bss_mult$`Coho_Adult_AD|UM|UNK|NA_Kept`)

```

```{r estimate_bss_multiple_est_cg, eval=FALSE}

estimates_bss_mult <- list()

for(ecg in names(inputs_bss_mult)) {
  gc(verbose = T)
  try(
    temp_fit <- fit_bss(
      bss_inputs_list = inputs_bss_mult[[ecg]],
      n_iter = 200
    )

    #do some thinning/summarization...
    #...should be a function
    keep_fit <- list()
    keep_fit$overview <- temp_fit |> 
...
estimates_bss |> 
  ...
  mutate(
    est_cg = inputs_bss$est_cg
  ) |> 
  left_join(
    bind_cols(
      estimates_pe$catch |> 
        group_by(est_cg) |>
        #filter(est_cg == inputs_bss$est_cg) |> 
        summarise(
          C_sum = sum(est),
          .groups = "drop"
        ),
...
estimates_bss |> 
  tidybayes::spread_draws(C_sum) |> 
  ggplot(aes(C_sum)) +
  ggdist::stat_halfeye()

estimates_bss |> 
  tidybayes::spread_draws(E_sum) |> 
  ggplot(aes(E_sum)) +
  ggdist::stat_halfeye()
...
estimates_bss |> 
  ...
  separate(
    col = indices,
    into = c("section_num", "day_index", "angler_type_ind"))

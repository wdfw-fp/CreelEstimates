...
date: "`r Sys.Date()`"
params:
  analysis_name: "District 17 South_Willapa River_winter_steelhead_2022"
...
pe$effort_census <- dplyr::left_join(
  ...
  dplyr::mutate(
    angler_type = dplyr::case_when(
      count_type %in% c("Shore - No Boat") ~ "bank",
      count_type %in% c("Shore - Pontoon/Kick/Kayak", "Shore - Drift/Raft", " Boat - Motor", "Shore - Motor Boat", "Boat - Drift Raft") ~ "boat" # EB relevel specific to Nasselle / Willapa angler type categories of interest and assessed in data collection
    )
  ) |> 
  # dplyr::mutate(
  #   angler_type = dplyr::case_when(
...
pe$interview <- dplyr::left_join(
  ...
  dplyr::mutate(
    ...
    angler_type = if_else(boat_used == "Yes", "boat", "bank"), # EB adjusted specifically for Nasselle / Willapa script
    ...
    total_group_count = angler_count, # EB adjusted specifically for Nasselle / Willapa script, did not use total group count field

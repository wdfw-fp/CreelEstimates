...
params:
  ...
  water_body: "Nooksack River - North Fork"
  date_start: "2022-05-28"   
  date_end: "2022-06-15"
  index_count_types: "Vehicle Only,Trailers Only"
  census_expansion: "direct"
  model_period: "Week"
  sections: "District 14_Nooksack River - North Fork_spring_chin_2022_sections.csv"
output:
  html_document:
    fig_caption: yes
    theme: default
...
lut <- map(
  list(
    river_loc = "input_files/river_locations_master.csv", # using file with Skagit location
    sections = file.path("input_files", params$sections),
    census_expansion = "input_files/District 14_Nooksack River - North Fork_spring_chin_2022_tiexp.csv" 
  ),
  ~readr::read_csv(file.path(.x))
)

lut$river_loc <- lut$river_loc |> filter(River == params$water_body)
  ...
  left_join(
    expand_grid(
      event_date = seq(
        ...
        by = "day")
      ,
      s = paste0("open_section_", sort(unique(lut$sections$section)))
    ) |> 
...
d_days <- rows_upsert(d_days,
    bind_rows(
      ...
      tibble(section = "1", closure_begin = "2022-05-27", closure_end = "2022-05-27")
...
point_estimate_data_prelim$effort_census <- point_estimate_data_prelim$effort_census |> 
  select(event_date, water_body, location, section, tie_in_indicator, count_type, count_quantity) |> 
  left_join(
    left_join(
...
point_estimate_data_prelim$effort_index <- point_estimate_data_prelim$effort_index |> 
  ...
  select(event_date, water_body, location, section, tie_in_indicator, count_sequence, count_type, count_quantity) |>
  left_join(d_days, by = "event_date") |> 
...
point_estimate_data_prelim$interviews_boat_effort <- point_estimate_data_prelim$interview |>
  ...
  summarize(
    daily_sum_angler = sum(angler_count),
    daily_sum_index_count_from_interview = sum(trailer_count), # EB 5/12/2022 found error, this was vehicles but is supposed to be trailers - this leads to problem with negative anglers when subtracting boat from total in Skagit fall 9/1 - 10/31 data 
    angler_hours_total = sum(angler_hours_total),
    anglers_per_index_count_from_interview = daily_sum_angler / daily_sum_index_count_from_interview) |>
  mutate(
    angler_type = "boat")
...
# |>
#   pivot_wider(names_from = angler_type, values_from = index_count) |>
#   mutate(
#     check = if_else(boat > total, "Yes", "No")
#   )
...
point_estimate_data_prelim$effort_interviews_totalandboat <- point_estimate_data_prelim$daily_effort_estimates |>
  ...
  mutate(
    mean_daily_TI_expan = mean_daily_effort * 1 ## workaround for initial Nooksack estimate, given first tie in survey was 0/0
  ) 
...
estimates$daily_effort_cpue_catch <- point_estimate_data_prelim$daily_CPUE  |>
  ...
  # ) |> 
    ...
    # filter(str_detect(catch_group, 'Chinook')) |> # filter to specific species
    # filter(str_detect(catch_group %in% c('Chinook', 'Steelhead')) |> # filter to specific species
  filter(catch_group %in% c("Chinook_Adult_UM_Released", "Chinook_Adult_AD_Kept","Steelhead_Adult_UM_Released")) |> # filter to specific catch_group
  distinct() |>
  # filter(!is.na(mean_daily_TI_expan)) |> 
...
estimates$time_strata_catch_total |>
  # filter(catch_group %in% c("Chinook_Adult_UM_Released", "Chinook_Adult_AD_Released")) |>
    filter(catch_group %in% c("Chinook_Adult_UM_Released", "Chinook_Adult_AD_Kept","Steelhead_Adult_UM_Released")) |>

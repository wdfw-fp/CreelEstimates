...
#!!see below. appears unneeded until estimate construction stage, better kept off until then
#!!pending final confirmation from EB/KB
    # |>
...
#     |> 
# #!!unclear that this is still needed given following drop_na & left_join...?
#       right_join(creel$days_section_angler_type, by = c("event_date", "section", "angler_type")) |>
#       select(time_strata, event_date, DayType, section, angler_type, everything()) |>
#       arrange(event_date)
...
# #!! DA seems like all of these can be avoided? 
# #!! Only used later in estimates, but are better kept separate until then...?
#   pe$angler_hours_daily_mean <- pe$angler_hours_daily_mean |>
...
#!!pending above...note still incorrect actual "df = " calc...will update if needed  
  ...
  #note the var() and sd() functions return NA when passed a length-1 vector (single obs)
#!! what exactly is the point of calc sample distributional moments when only n=1-2 per DayType per week?
...
#!!adding PER-DAY the week-daytype-section-angler_type strata moments
#!!but could do this with different, coarser strata
#!!for example, filling "ang_hrs" from mean over more weeks or both angler_types or etc...
    group_by(time_strata, DayType, section, angler_type) |>
    mutate(
      across(
...
# various aggregations follow easily ...
  ests$effort |> summarise(across(ang_hrs, sum), .groups = "drop")
  ests$effort |> group_by(DayType) |> summarise(across(ang_hrs, sum), .groups = "drop")
  ests$effort |> group_by(angler_type) |> summarise(across(ang_hrs, sum), .groups = "drop")
  ests$effort |> group_by(DayType, angler_type) |> summarise(across(ang_hrs, sum), .groups = "drop")
  ests$effort |> group_by(time_strata) |> summarise(across(ang_hrs, sum), .groups = "drop")
  ests$effort |> group_by(time_strata, DayType) |> summarise(across(ang_hrs, sum), .groups = "drop")
  ests$effort |> group_by(time_strata, angler_type) |> summarise(across(ang_hrs, sum), .groups = "drop")
  ests$effort |> group_by(time_strata, DayType, section, angler_type) |> summarise(across(ang_hrs, sum), .groups = "drop")
#but presumably desired to include dispersion however...
#and could revise above group_by to "fill" unsampled ang_hrs with week-daytype-section-angler_type means 
#illustrating a concept/option...possibly closer to the original conception
#here, total_effort is identical to ang_hrs in the final group_by/summarize above
  ests$effort |> 
    group_by(time_strata, DayType, section, angler_type) |>
    summarize(
      across(
    ...
    right_join(creel$days_total, by = c("time_strata", "DayType", "section")) |> 
    left_join(pe$df, by = c("time_strata", "DayType", "section", "angler_type")) |> 
    mutate(
      total_effort = N_days * ang_hrs_mean,
      #!!need clarification on "variance_total_daily_effort"
      variance_total_daily_effort = if_else(
        n_days_samp < N_days,
        (N_days^2) * (ang_hrs_var / n_days_samp) * (1-(n_days_samp/N_days)),
        (N_days^2) * (ang_hrs_var / n_days_samp)
      )
    )
  
  
#!!not ready yet to add these back...

...
# 
# lu_input$river_lonlat <- dplyr::filter(lu$river_lonlat, River %in% input$water_body) |> dplyr::distinct(District, .keep_all = T)
# lu_input$sections <- dplyr::select(lu$sections[[input$sections]], water_body_desc, location = location_code, section)
# lu_input$census_exp <- if(str_detect(input$index_count_types, "Vehicle|Trailer")) {

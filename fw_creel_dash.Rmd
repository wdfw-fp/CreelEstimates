...
# pe <- list()
# 
# input$analysis_name <- input_analysis_levels$analysis_name[2]
# 
...
#!!pending above...note still incorrect actual "df = " calc...will update if needed  
  ...
  pe$est_effort_s_ts_dt_at <- left_join(
    ...
    pe$angler_hours_daily_mean |> select(section, event_date, angler_type, ang_hrs_daily_mean_TI_expan)
    ,
    by = c("section", "event_date", "angler_type")
  ) |>
...
#!! such that case logic prevents 0 total_effort_var at n_obs==N_days 
    mutate(
      ...
      effort = N_days * ang_hrs_mean,
      effort_var = if_else(
        n_obs < N_days,
        (N_days^2) * (ang_hrs_var / n_obs) * (1-(n_obs/N_days)),
        (N_days^2) * (ang_hrs_var / n_obs)
      ),
      effort_l95 = effort - qt(1-(0.05/2),df)*(effort_var^0.5),
      effort_u95 = effort + qt(1-(0.05/2),df)*(effort_var^0.5)
    ) 
  ...
  pe$est_catch_s_ts_dt_at <- left_join(
    ...
    mutate(
      ...
      catch = N_days * catch_est_mean,
      catch_var = if_else(
        n_obs < N_days,
        (N_days^2) * (catch_est_var / n_obs) * (1-(n_obs/N_days)),
        (N_days^2) * (catch_est_var / n_obs)
      ),
      catch_l95 = catch - qt(1-(0.05/2),df)*(catch_var^0.5),
      catch_u95 = catch + qt(1-(0.05/2),df)*(catch_var^0.5)
    ) 
...
#   summarise(across(c(effort, effort_var), sum), .groups = "drop")
#   

    #by angler_type, summed over weekday/weekend and week/month
    pe$est_effort_s_ts_dt_at |>
      group_by(section, angler_type) |>
      summarise(across(c(effort, effort_var), ~round(sum(., na.rm = T))), .groups = "drop") |> 
      gt() |> 
      gt::summary_rows(columns = effort, fns = list(sum = ~round(sum(., na.rm = T),0)))
    
# pe$est_effort_s_ts_dt_at |> 
#   group_by(section) |> 
#   summarise(across(c(effort, effort_var), ~round(sum(.))), .groups = "drop")
  ...
  if(!is.null(pe$est_effort_s_ts_dt_at)){
    (
      ...
      ggplot(pe$est_effort_s_ts_dt_at,
             aes(time_strata, effort, fill = angler_type)) +
        geom_col(position = "dodge") +
        #scale_x_date("", date_breaks = "1 week", guide = guide_axis(n.dodge = 2)) +
        scale_y_continuous("Hours") +
      ...
      ggplot(pe$est_effort_s_ts_dt_at |> 
               group_by(DayType, angler_type) |> mutate(effort_cml = cumsum(effort)) |> ungroup(),
...
### by catch_group {data-height=1000}

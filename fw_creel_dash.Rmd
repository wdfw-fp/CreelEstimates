...
river_lonlat <- reactiveValues() #filtered from lu$river_lonlat tibble
sections <- reactiveValues() #pulled from lu$sections list of tibbles
census_exp <- reactiveValues() #pulled from lu$census_expansions list of tibbles
closures <- reactiveValues() #pulled from lu$closures list of tibbles
...
# 
# river_lonlat <- dplyr::filter(lu$river_lonlat, River %in% input$water_body) |> dplyr::distinct(District, .keep_all = T)
# sections <- dplyr::select(lu$sections[[input$sections]], water_body_desc, location = location_code, section)
# census_exp <- if(str_detect(input$index_count_types, "Vehicle|Trailer")) {
#     lu$census_expansions[[input$census_exp]] |>
#       mutate(
...
#     }
# closures <- lu$closures_by_section[[input$closures]]
...
reactive({
  ...
  if(str_detect(input$index_count_types, "Vehicle|Trailer")) {
    census_exp <- lu$census_expansions[[input$census_exp]] |> 
      mutate(
        angler_type = if_else(angler_type == "bank", "total", "boat"),
        cen_exp_meth = input$census_expansion
      )

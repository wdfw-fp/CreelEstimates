...
#!!make sure fishing_end_time logic is working as intended for eg Kalama
  ...
  #   then angler_hour daily means require first using interviews to estimate anglers_per_vehicle by angler_type total & boat 
  #   so anglers_per_vehicle can be multiplied against the trailer & vehicle counts in effort_index, releveled to boat/total
  ...
  if(str_detect(input$index_count_types, "Bank|Boat")) {
    ...
    #count_census begins summed by event_date, section, tie_in_indicator, count_sequence, angler_type [bank, boat]
    #count_index begins summed by event_date, section, count_sequence, count_type [Bank Ang, Boat Ang, Vehic, Trailr] and derived angler_type [bank, boat, total])
    pe$census_TI_expan <- left_join(
      pe$effort_census,
      pe$effort_index,
      by = c("event_date", "section", "count_sequence","angler_type")
      ) |>
#!!removed unused TI_expan_mean = mean(TI_expan, na.rm=TRUE) from summarize, which also allows dropping unused mutated TI_expan
#!!and then is remaining mutate(count_index) ever necessary?
      mutate(count_index = replace_na(count_index, 0)) |>
    ...
    pe$angler_hours_daily_mean <- left_join(
      ...
      mutate(creeled = if_else(!is.na(ang_hrs_daily_mean_TI_expan) & open_section_1 == TRUE, "Y", "N")) |>
      arrange(event_date)
...
#!!presumably this blows up if sum(vehicle_count)==0, but that is non-issue?
    pe$interview_ang_per_vehic <- bind_rows(
      pe$interview |>
        group_by(event_date, section) |>
        summarize(angler_type = "total", anglers_per_vehicle = sum(angler_count) / sum(vehicle_count), .groups = "drop")
      ,
      pe$interview |>
        filter(angler_type == "boat") |>
        group_by(event_date, section) |>
        summarize(angler_type = "boat", anglers_per_vehicle = sum(angler_count) / sum(vehicle_count), .groups = "drop")
    ) |> 
      arrange(event_date, section)
    
#!!this left_join drops rows of index counts where a given date-section-angler_type is missing from interview data
    pe$angler_hours_daily_mean <- left_join(
      ...
      mutate(angler_hours_daily_mean = anglers_per_vehicle * count_index_mean * DayL) |>
      arrange(event_date, section)
    
    pe$census_TI_expan <- left_join(
      #census already grouped & summed by event_date, section, tie_in_indicator, count_sequence, and angler_type [bank, boat]
      #but here first split out boat, collapse and then reassign angler_type to total & boat
      bind_rows(
        pe$effort_census |>
...
#!!note above drop of unused TI_expan & TI_expan_mean
      mutate(count_index = replace_na(count_index, 0)) |>
      group_by(section, angler_type) |>
      summarise(
        TI_expan_weighted = sum(count_census) / sum(count_index),
...
#!!this creates negatives when large (direct) TI-expan for boat expand angler_hours > total
#!!explored calc'ing bank angler_hours sooner (since present in interview and census itself)
#!!but that would currently complicate/prevent join of TI-expan table
#!!NEEDS ATTENTION
    if(any(pe$interview$angler_type=="boat")) {
      pe$angler_hours_daily_mean <- bind_rows(
        #total and boat
        pe$angler_hours_daily_mean |>
          select(event_date, section, angler_type, ang_hrs_daily_mean_TI_expan),
        #derived bank
        pe$angler_hours_daily_mean |>
          select(event_date, section, angler_type, ang_hrs_daily_mean_TI_expan) |> 
          pivot_wider(names_from = angler_type, values_from = ang_hrs_daily_mean_TI_expan, values_fill = 0) |>
          mutate(
            angler_type = "bank",
            ang_hrs_daily_mean_TI_expan = total - boat,
      ...
      pe$angler_hours_daily_mean <- bind_rows(
        ...
        pe$angler_hours_daily_mean |>
          ...
          mutate(angler_type = "boat", ang_hrs_daily_mean_TI_expan = 0),
        pe$angler_hours_daily_mean |>
          select(event_date, section, ang_hrs_daily_mean_TI_expan) |>
          mutate(angler_type = "bank")
      )
    }    

#!!unclear that all of this is still needed given following drop_na & left_join...
    pe$angler_hours_daily_mean <- pe$angler_hours_daily_mean |> 
      ...
      mutate(creeled = if_else(!is.na(ang_hrs_daily_mean_TI_expan), "Y", "N")) |>
      select(time_strata, event_date, DayType, section, angler_type, everything()) |>
      arrange(event_date)
  ...
  pe$angler_hours_daily_mean <- pe$angler_hours_daily_mean |>
    ...
    #add total days per DayType in week/month strata
    left_join(
      creel$days_total, 
      by = c("time_strata", "DayType", "section")) |>
    #add number of days sampled within that dayType-time_strata
    left_join(
      pe$angler_hours_daily_mean |>
        filter(!is.na(ang_hrs_daily_mean_TI_expan) & creeled == "Y") |>
        count(time_strata, DayType, section, angler_type, name = "n_days"),
      by = c("time_strata", "DayType", "section", "angler_type")) |>
    #add total days per DayType across time_strata
    left_join(
      creel$days_total |>
        group_by(section, DayType) |>
        summarise(N_days_total = sum(N_days), .groups = "drop"),
      by = c("DayType", "section")) |>
    #add total days sampled by DayType across time_strata
    left_join(
      pe$angler_hours_daily_mean |>
        filter(!is.na(ang_hrs_daily_mean_TI_expan) & creeled == "Y") |>
        count(time_strata, DayType, section, angler_type, name = "n_days") |>
  ...
  pe$angler_hours_daily_mean_strata_mean <- pe$angler_hours_daily_mean |>
    #!!Not including DayType in group_by?
    group_by(time_strata, section, angler_type) |>
    mutate(mean_strata_effort = mean(ang_hrs_daily_mean_TI_expan)) |>
    ungroup() |>
    #!!is this complete actually necessary??
    complete(time_strata, section, angler_type, fill = list(mean_strata_effort = 0)) |>
    mutate(
      ang_hrs_daily_mean_TI_expan = if_else(
        ang_hrs_daily_mean_TI_expan == 0,
        mean_strata_effort,
...
#!!need to add logic to avoid NaN if below if_else evals false AND any angler_hours_strata_mean==0?
    ...
    summarise(
      n_groups = n(),
#!!the cpue_rom here is a ratio of sums - are we calling that equivalent to "ratio of the means"?
      cpue_rom_daily = sum(fish_count) / sum(angler_hours_total), # ratio of the means CPUE estimator
...
#!!DA pending conversation with EB this can be directly piped from above pe$interview_daily_totals and eliminate intermediary
  ...
  pe$daily_effort_cpue_catch <- left_join(
    pe$daily_CPUE,
    pe$angler_hours_daily_mean_strata_mean,
    ...
    mutate(
      ...
      catch_estimate = if_else(
        ...
        ang_hrs_daily_mean_TI_expan * cpue_mor_daily,
        ang_hrs_daily_mean_TI_expan * cpue_mor_strata_mean
      ),
      # catch_estimate = if_else(catch_dailysum > 0 & ang_hrs_daily_mean_TI_expan == 0, cpue_mor * mean_strata_effort, catch_estimate) # circle bacl to this, issue is days with non-zero catch by no effort
      # catch_estimate_rom = if_else(!is.na(cpue_rom), ang_hrs_daily_mean_TI_expan * cpue_rom, ang_hrs_daily_mean_TI_expan * mean_strata_cpue_rom), # option to use ratio of the means CPUE estimator
    ...
    distinct() |>
    # filter(!is.na(ang_hrs_daily_mean_TI_expan)) |>

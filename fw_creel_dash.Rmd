...
```{r input_selectors}
selectInput(inputId = 'analysis_name', label = 'Analysis Name', choices = input_levels$analysis_name, selected = input_levels$analysis_name[1])
...
# input <- list()
# 
# input$analysis_name <- input_levels$analysis_name[1]
# 
...
# # input$water_body <- c("Skagit River")
# # input$dates <- c(as.Date("2022-05-01", "%Y-%m-%d"), as.Date("2022-05-31", "%Y-%m-%d"))
# 
...
reactive({
  ...
  input_analysis <- dplyr::filter(input_levels, analysis_name == input$analysis_name)
  ...
  lu_input$census_exp <- mutate(
    lu$census_expansions[[input$analysis_name]],
    cen_exp_meth = input$census_expansion)
  #lu$census_expansions[[input$census_exp]] |> mutate(cen_exp_meth = input$census_expansion)
  
  lu_input$closures <- drop_na(lu$closures_by_section[[input$analysis_name]], event_date)
    #lu$closures_by_section[[input$closures]] |> drop_na(event_date)

  
  creel$event <- paste0(
    dwg_base$event,
    "?$where=project_name in('", input_analysis$proj_name, "')", #input$proj_name, "')",
    " AND water_body in('", paste0(input_analysis$water_body, collapse = "','"), "')", #paste0(input$water_body, collapse = "','"), "')",
    " AND event_date between '", first(lu_input$closures$event_date), #input$dates[1],
    "T00:00:00' and '", last(lu_input$closures$event_date), #input$dates[2],
  ...
  if(nrow(creel$event) > 0) {
    ...
    creel$days <- tibble::tibble(
      event_date = lu_input$closures$event_date, #seq.Date(input$dates[1], input$dates[2], by = "day"),
...
#!!pending above...note still incorrect actual "df = " calc...will update if needed  
  ...
  pe$angler_hours_daily_mean |> 
    ...
    mutate(ah = ang_hrs_daily_mean_TI_expan, d = fct_reorder(as.character(event_date), ah, max)) |> 
    ggplot(aes(d, ah, fill = angler_type)) + geom_col(position = "dodge")
  pe$angler_hours_daily_mean |> 
    select(event_date, section, angler_type, ang_hrs_daily_mean_TI_expan) |> 
    ggplot(aes(event_date, ang_hrs_daily_mean_TI_expan, fill = angler_type)) + geom_col(position = "dodge") +scale_x_date(date_breaks = "3 days")
  ests$effort |> 
    ggplot(aes(event_date, ang_hrs_daily_mean_TI_expan, fill = angler_type)) + 
    geom_col(position = "dodge") +scale_x_date(date_breaks = "1 day") +
    #facet_wrap(~section+time_strata, scales = "free_x")
    facet_grid(rows = vars(section, time_strata), cols = vars(DayType), scales = "free_x")
  ests$effort |> 
    count(section, time_strata, DayType, angler_type) |> pivot_wider(names_from = angler_type, values_from = n)
  ...
  ests$effort_ts_s_at_dt_mean_filled <- ests$effort |> 
    ...
    mutate(
      ...
      ang_hrs_var = var(ang_hrs_daily_mean_TI_expan, na.rm = T),
...
ests$effort_s_at
  ...
  ests$effort_s_at_dt <- ests$effort |> 
...
ests$effort_s_at_dt |> summarise(across(total_effort, sum), .groups = "drop")

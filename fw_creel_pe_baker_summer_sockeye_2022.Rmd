...
date: "`r Sys.Date()`"
params:
  analysis_name: "District 14_Baker Lake_summer_sockeye_2022"
...
creel$days <- tibble::tibble(
  ...
  DayL = suncalc::getSunlightTimes(
    ...
    pluck("DayL"),
  #Monday to Sunday stat weeks
  Week = 1 + as.numeric(format(event_date, "%W")) - as.numeric(format(as.Date(cut(event_date, "year")), "%W")),
...
pe$effort_index <- dplyr::filter(
  ...
  dplyr::group_by(section, DayType, event_date,location, count_sequence, count_type) |> # group at location level to perform boat count correction to trailer count at location-count_sequence pairs
  dplyr::summarise(count_index = sum(count_quantity), .groups = "drop") |>
  tidyr::pivot_wider(names_from = count_type, values_from = count_index, values_fill = 0) |>
  dplyr::mutate(
  ...
  tidyr::pivot_longer(cols = c(`Trailers Only`,Boats), names_to = "count_type", values_to = "count_quantity") |> 
  dplyr::filter(!count_type %in% c("Boats")) |> 
...
pe$interview <- dplyr::left_join(
  ...
  dplyr::mutate(
    ...
    angler_hours_total = total_group_count * angler_hours # changed to total_group_count to match census count protocol
  ) |>
...
####  } else if(str_detect(params$index_count_types, "Vehicle|Trailer")) {
  ...
  pe$interview_ang_per_vehic <- bind_rows(
    pe$interview |>
      ...
      summarize(angler_type = "total", anglers_per_vhcl_trlr = sum(total_group_count) / sum(vehicle_count), .groups = "drop")
    ,
    pe$interview |>
      filter(angler_type == "boat") |>
      #could re-introduce here: group_by(section)
      summarize(angler_type = "boat", anglers_per_vhcl_trlr = sum(total_group_count) / sum(trailer_count), .groups = "drop")
    )
...
#     pe$census_TI_expan <- lu_input$census_exp |>
#       select(cen_exp_meth, angler_type, section, p_TI, TI_expan_indirect = Indirect_TI_Expan) |> 
#       mutate(
#         cen_exp_meth = replace_na(cen_exp_meth, params$census_expansion),
#         p_TI = replace_na(p_TI, 1),
#         TI_expan_indirect = as.numeric(TI_expan_indirect) |> replace_na(1),
#         TI_expan_final = TI_expan_indirect)
#         
#       # )
#   # }
#   
...
pe$daily_cpue_catch_est <- pe$interview_and_catch |>
  ...
  summarise(
    # cpue_mor_daily = mean(cpue_interview),
            total_fish = sum(fish_count),
            total_angler_hours = sum(angler_hours_total),
            cpue_rom = total_fish / total_angler_hours,
            .groups = "drop") |> 
  left_join(
    pe$angler_hours_daily_mean, 
  ...
  mutate(catch_estimate = round(cpue_rom * angler_hours_daily_mean, 3)) |>
...
pe$est_effort_s_ts_dt_at <- left_join(
  ...
  pe$angler_hours_daily_mean |> select(section, DayType, event_date, angler_type, angler_hours_daily_mean)
  ,
  by = c("section", "DayType", "event_date", "angler_type")
) |>
  group_by(section, time_strata, DayType, angler_type) |>
  summarize(
    n_obs = sum(!is.na(angler_hours_daily_mean)), #n_days = n(),
    across(
      .cols = c(angler_hours_daily_mean),
      .fns = list(
        mean = ~mean(.x, na.rm = T),
        var = ~var(.x, na.rm = T)
...
sockeye_harvest_groups <- c("Sockeye_Adult_UM_Kept", "Sockeye_Adult_AD_Kept", "Sockeye_Jack_AD_Kept", "Sockeye_Jack_UM_Kept", "Sockeye_Adult_UNK_Kept")

pe$interview_and_catch |>
  filter(catch_group %in% sockeye_harvest_groups) |>
  group_by(section, time_strata, DayType, event_date, angler_type, catch_group) |> 
...
pe$angler_hours_daily_mean |> 
  ggplot(aes(event_date, angler_hours_daily_mean, fill = angler_type)) +
...
left_join(
  lu_input$sections |> filter(location %in% c("Baker Lake")) |> 
    select(section,location)
) |> 
  select(section, location, catch, var, se)

gt(d1, rowname_col = "rowname") |> 
  gt::fmt_integer(columns = where(is.numeric)) |> 
  gt::tab_options(container.overflow.x = TRUE, container.overflow.y = TRUE)

```

### Sockeye weekly estimates

```{r sockeye_weekly}
d <- pe$est_catch_s_ts_dt_at |> filter(catch_group %in% sockeye_harvest_groups)
bind_rows(
...
### Sockeye daily estimates

```{r sockeye_total_weekly}
d1 <- pe$est_catch_s_ts_dt_at |> 
  filter(catch_group %in% sockeye_harvest_groups & section == 1) |>
  group_by(section, time_strata, DayType) |> 
  summarise(
    catch_est_mean = sum(catch_est_mean)
  ) |> 
  left_join(creel$days |> 
              filter(open_section_1 == TRUE) |> 
              select(event_date, DayType, time_strata)) |> 
  group_by(event_date, time_strata, DayType) |>
  summarise(
    daily_est = sum(catch_est_mean)
  ) |> 
  mutate(across(where(is.numeric), round, 0)) |> 
  ungroup() |> 
  write_csv("model_output/baker_sockeye_daily_estimates.csv")

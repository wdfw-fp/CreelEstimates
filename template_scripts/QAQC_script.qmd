...
#define a threshold for the "zoom"
threshold <- 5  # Values > 5 times the mode
...
#plot histogram
fish_count_plot <- ggplot(data = dwg$catch, aes(x = fish_count)) +
  geom_histogram(bins = 25, color = "black", fill = "blue") +
  geom_text(data = filter(dwg$catch, !is.na(far_from_peak)), 
            aes(x = fish_count, y = 0, label = far_from_peak),
            vjust = -0.5, color = "red", size = 3) +
  theme_classic()

#create zoomed-in version for small values
fish_count_zoomed_plot <- ggplot(data = filter(dwg$catch, fish_count > peak_value$fish_count * threshold),
                      aes(x = fish_count)) +

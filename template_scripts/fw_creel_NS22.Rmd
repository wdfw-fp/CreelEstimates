...
dwg$effort <- dwg$effort |>
  ...
  # filter(count_sequence <= 4) |> # handful of 4th index counts that don't align with bulk of the dataset
  mutate(
    section_num = case_when(
    section_num == 2 ~ 1,
    section_num %in% c(3,4) ~ 2
    ),
    p_census_bank = 1, #coerce all rows to 1 - update 4/25/24 fixed in database so shouldn't need this 
    p_census_boat = 1
  ) 

# perform the same section reassignment and aggregation as above for the effort data

# instances where vehicle count > angler count appear to be a problem for the BSS model (also logicall problematic), removing those records for now
# but should be reviewed in QA/QC 

## 4/25/2024 check to ensuer that angler_count is the field we want to use (vs. total group count), check and see what field is being used in the prep_bss_inputs function !!

dwg$interview <- dwg$interview |>
  filter(!section_num == 1) |> # remove section 1 (only 4 interviews)
  mutate(
    section_num = case_when(
    section_num == 2 ~ 1,# make section 2 section 1
    section_num %in% c(3,4) ~ 2 # aggregate section 3,4 to section 2
    ),
    p_census_bank = 1, #coerce all rows to 1
    p_census_boat = 1 #coerce all rows to 1
    # sumvehicletrailer = rowSums(across(c(vehicle_count, trailer_count))) #add vehicle and trailer counts / interview
  ) |> 
  rowwise() |> 
  mutate(cond = vehicle_count > angler_count) |> # mutate new boolean field to show if an interview meets condition of vehicles   
  ungroup() |> 
  filter(cond == FALSE) # removing interviews where vehicle count > angler count 
  

# Aggregating sections 2 and 3 necessitates dropping section 3 from closures, since 3 is no longer present in the effort and interview data

dwg$closures <- dwg$closures |>
  filter(!section_num %in% c(3,4))


## check if this is necessary !
# |> 
#   filter(!sumvehicletrailer == 0) |>  # filter out interviews with no vehicles or trailers (causing error fitting bss model)
#   select(-sumvehicletrailer)


dwg$days <- dwg$days |>
  select(!c(open_section_3, open_section_4))

# dwg$days <- dwg$days |>
#   select(!c(open_section_2, open_section_3)) 

# add +1 dummy vehicle counts to trailer counts
...
dwg$effort <- index_nonzero_trailers |> 
  rbind(index_no_trailers, census) |> 
  arrange(event_date, count_sequence)

```

```{r aggregating sections 1 and 2, eval=TRUE}

## aggregating all data to single section

dwg$effort <- dwg$effort |>
  mutate(section_num = 1)

dwg$interview <- dwg$interview |>
  mutate(section_num = 1)

dwg$days <- dwg$days |>  select(-open_section_2)

dwg$closures <- dwg$closures |> filter(!section_num == 2)
```
...
for(ecg in names(inputs_bss)) {
  ...
  ecg_fit <-
    fit_bss(
      ...
      adapt_delta = 0.7,
      max_treedepth = 12,
      threads_per_chain = 2,  # within-chain parallelization via reduce_sum
      init = "0",
      overwrite_exe = FALSE,  # reuse cached exe across catch groups; set TRUE to force recompile
      save_exe = TRUE,        # archive per-catch-group exe to outputs_dir for future reloading
      outputs_dir = outputs_folders$outputs
    )
...
overview_combined |> 
  ...
  fmt_number(-c(variable, est_cg, catch_group), decimals = 1) |> 
  tab_style(
    style = cell_fill(),

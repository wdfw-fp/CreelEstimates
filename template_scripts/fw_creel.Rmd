...
#### Hoh regulation comparison of bss results

##### Regime comparison (regulatory period effects)

Where a regulation (e.g. a boat-fishing restriction) applies unevenly across sections and days, comparing raw total catch between "open" and "closed" periods is confounded by unequal numbers of fishing days per stratum. `get_bss_regime_comparison()` normalizes to a per-day rate before comparing, and decomposes the per-day catch difference into effort/day and CPUE (catch rate) components, computed draw-by-draw to avoid transformation bias. See `R_functions/get_bss_regime_comparison.R` for full documentation.

This section is fishery-specific (only meaningful where a within-season regulatory contrast exists) — adjust or remove the `day_groups` construction below for fisheries without such a contrast.

```{r bss_regime_comparison, eval=TRUE, cache=params$enable_cache, dependson="estimates_bss", cache.extra=param_hash}
...
if (!exists("hoh_boat_reg")) {
  hoh_boat_reg <- dwg$days |>
    ...
    dplyr::select(section_num, boat_allowed)
}

# ---- Run the comparison per catch group ----
regime_comparison <- list()

for (ecg in names(inputs_bss)) {
  regime_comparison[[ecg]] <- get_bss_regime_comparison(
    draws_bss     = draws_bss,
    dwg           = dwg,
    inputs_bss    = inputs_bss[[ecg]],
    ecg           = ecg,
    day_groups    = hoh_boat_reg,
    regime_levels = c(TRUE, FALSE),  # TRUE (open) minus FALSE (closed)
    return_draws  = FALSE
  )
  estimates_bss[[ecg]]$regime_comparison <- regime_comparison[[ecg]]
  cli::cli_alert_success("Regime comparison complete for catch group: {ecg}")
}

# Save regime comparison results
saveRDS(regime_comparison, file.path(outputs_folders$outputs, "regime_comparison.rds"))
```

##### Regime comparison summary table

```{r table_regime_comparison, results='asis'}
# Display the draw-wise regime difference for each catch group: effort/day,
# catch/day, and CPUE, with sparse cells flagged.
for (ecg in names(regime_comparison)) {
  tbl <- regime_comparison[[ecg]]$comparison |>
    dplyr::filter(metric %in% c("diff_effort_per_day", "diff_catch_per_day", "diff_cpue")) |>
    dplyr::mutate(
      metric = dplyr::case_match(
        metric,
        "diff_effort_per_day" ~ "Effort (angler-hrs/day)",
        "diff_catch_per_day"  ~ "Catch (fish/day)",
        "diff_cpue"           ~ "CPUE (fish/angler-hr)"
      )
    ) |>
    dplyr::select(section_num, angler_final, metric, mean, lower, upper, n_days_a, n_days_b, sparse) |>
    gt::gt(groupname_col = "section_num") |>
    gt::fmt_number(columns = c(mean, lower, upper), decimals = 2) |>
    gt::cols_label(
      angler_final = "Gear",
      metric = "Metric",
      mean = "Mean diff (open - closed)",
      lower = "2.5%",
      upper = "97.5%",
      n_days_a = "Days open",
      n_days_b = "Days closed",
      sparse = "Sparse"
    ) |>
    gt::tab_header(
      title = glue::glue("Catch Group: {ecg}"),
      subtitle = "Draw-wise difference in per-day rates, open minus closed regime"
    ) |>
    gt::tab_style(
      style = gt::cell_fill(color = "lightyellow"),
      locations = gt::cells_body(rows = sparse)
    )
  cat(gt::as_raw_html(tbl))

  safe_name <- stringr::str_replace_all(ecg, "[^[:alnum:]]", "_")
  save_gt_table(tbl, paste0("regime_comparison_", safe_name))
}
```

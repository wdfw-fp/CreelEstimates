...
params:
  ...
  fishery_name: "Humptulips salmon 2022"
  est_date_start: "2022-09-15"
  est_date_end: "2022-09-30"
  est_catch_groups: !r data.frame(rbind(
    c(species = 'Chinook', life_stage = 'Adult', fin_mark = 'UM', fate = 'Kept'),
...
<!-- `r paste(params$fishery_name)`{.unnumbered .unlisted} -->
...
```{r load_functions, cache=TRUE, dependson="setup"}
# Load functions (pre-package method)
walk(list.files(here("R_functions"), full.names = T), source)
```

#### Establish fishery folder
```{r define_fishery_folder, eval=TRUE}
...
#### Establish file structure for outputs and cache
```{r establish_file_structure, cache=TRUE, cache.extra=param_hash}
...
#### Helper functions to save plots/tables
...
## Download data
```{r dwg_fetch, cache=TRUE, results='hide', dependson="establish_file_structure", cache.extra=param_hash}
...
map(
  ~plot_inputs_pe_cpue_period(
    ...
    period_pe = params$period_pe,
    outputs_folders = outputs_folders,
    filename = paste0("pe_cpue_estimates_", .x)
  )
)
```
...
map(
  ~plot_est_pe_catch(
    ...
    period_pe = params$period_pe,
    outputs_folders = outputs_folders,
    filename = paste0("pe_catch_estimates_", .x))
)
```

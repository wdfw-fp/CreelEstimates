...
params:
  ...
  fishery_name: "Chehalis winter steelhead 2024-25"
  est_date_start: "2024-11-01"
  est_date_end: "2025-02-28"
...
# devtools::install_github("wdfw-fp/creelutils", force = TRUE)
library(creelutils)
...
## Create empty list to store important R objects
saved_r_objects <- list() 

## Collapse catch group into one string, e.g., "Coho_Adult_UM_Released"
# assumes one catch group per run
catch_group <- params$est_catch_groups |> unite("cg", everything(), sep = "_", remove = TRUE) |> pull(cg)
saved_r_objects$catch_group <- catch_group

## Base folder
base_path <- here("fishery_analyses", params$project_name, params$fishery_name, catch_group)
...
path_components <- list(
  ...
  base_path, # Create subfolders within catch group 
  file.path(base_path, "diagnostics"),
  file.path(base_path, "outputs")
)
...
saved_r_objects$analysis_lut <- analysis_lut
```

# `r paste(params$fishery_name)`

```{r}
saved_r_objects$table_params <- tibble(param = names(params), value = as.character(params))
saved_r_objects$table_params |> gt()
```
...
dwg$days <- prep_days(
  ...
  sections = unique(dwg$fishery_manager$section_num),
...
saved_r_objects$table_sections <-
  dwg$effort |> 
...
saved_r_objects$table_sections |> gt()
```
...
saved_r_objects$table_period_date_reference <-  
  dwg$days |> 
      group_by(period) |> 
      summarise(
...
saved_r_objects$table_effort_counts <-
  dwg$effort |>
...
saved_r_objects$table_effort_paired <- 
  left_join(
    dwg$effort |>
  ...
  mutate(census_total = n_distinct(event_date))
...
saved_r_objects$table_interview_counts <-
  dwg$interview |>
    filter(between(event_date, as.Date(params$est_date_start), as.Date(params$est_date_end))) |>
...
saved_r_objects$table_interviews_by_date <-
  dwg$interview |>  
    filter(between(event_date, as.Date(params$est_date_start), as.Date(params$est_date_end))) |> 
...
saved_r_objects$table_raw_catch_sum <-
  dwg$catch |>
...
saved_r_objects$dwg_summ <- dwg_summ
```
...
saved_r_objects$plot_index_effort_counts <-
...
saved_r_objects$inputs_pe <- inputs_pe
```
...
saved_r_objects$table_paired_census_index_counts <-
inputs_pe$paired_census_index_counts |> 
...
saved_r_objects$plot_paired_census_index_counts <-
...
saved_r_objects$estimates_pe <- estimates_pe
```
...
saved_r_objects$table_effort_summary <-
  estimates_pe$effort |>
...
saved_r_objects$plot_pe_effort_estimates <-
  plot_est_pe_effort(
...
saved_r_objects$plot_pe_cpue_period <- 
...
```{r PE_catch_summary}
saved_r_objects$table_pe_catch_summary <-
  estimates_pe$catch |> 
...
saved_r_objects$plot_pe_catch_estimates <-  
  pe_catch_est_plots <- 
...
saved_r_objects$inputs_bss <- inputs_bss
```
...
```{r}
# Separate out draws - large file size
draws_bss <- list()

# Convert stanfit object to posterior draws
draws_array <- posterior::as_draws_array(ecg_fit)
...
# Subset columns down from thousands to parameters of interests
draws_trimmed <- posterior::subset_draws(draws_array, variable = params_keep)

draws_bss$draws_df <- posterior::as_draws_df(draws_trimmed) |>  #could supply variables of interest to trim (and speed up summary calculation)
  relocate(c(".draw", ".iteration", ".chain"))

# Summarize draws (inc. Rhat, ESS, etc)
draws_bss$summary_df <- posterior::summarize_draws(
  draws_bss$draws_df, 
  measures = list(mean, sd, mad, posterior::quantile2, rhat, ess_bulk, ess_tail)
)

colnames(draws_bss$summary_df) <- c("variable", "mean", "sd", "mad", "q5", "q95", "rhat", "ess_bulk", "ess_tail")

saveRDS(
  draws_bss,
  file = paste0(file.path(base_path), "/draws_bss.rds")
)
```

## Model Diagnostics

```{r summary_and_convergence}

draws_problems <- draws_bss$summary_df |> 
  filter(rhat > 1.009 | ess_bulk < 400 | ess_tail < 400) |> 
  mutate(
...
# Summary info
n_problems <- nrow(draws_problems)
pct_problems <- round(n_problems / nrow(draws_bss$summary_df) * 100, 1)

# gt table
saved_r_objects$table_bss_convergence <- 
  draws_problems |> 
    slice_head(n = 10) |> 
    select(-rhat) |> 
    relocate(rhat_display, .after = "variable")

# Add metadata
attr(saved_r_objects$table_bss_convergence, "analysis_id") <- analysis_lut$analysis_id

# Print
gt_tab <- saved_r_objects$table_bss_convergence |> 
  gt() |> 
  cols_label(variable = "Parameter", rhat_display = "R̂", ess_bulk = "ESS_bulk", ess_tail = "ESS_tail") |> 
  tab_header(
    title = "Posterior Summary Statistics and Convergence Diagnostics",
    subtitle = "Parameters shown meet at least one criteria: R̂ ≥ 1.01 or effective sample size (ESS) < 400"
  ) |> 
  tab_style(
  ...
  tab_source_note(
    ...
    "Magnitude: {n_problems} parameters ({pct_problems}%) flagged from {nrow(draws_bss$summary_df)} total.")
  ) |> 
  tab_footnote(
    footnote = "Filtered to 10 rows, sorted by descending R̂",
    locations = cells_column_labels(columns = c(rhat_display))
  )
...
gtsave(
  gt_tab, 
  filename = "table_bss_convergence_alerts.rtf", 
  path = file.path(base_path, "diagnostics"),
  expand = 10
)
```
...
saved_r_objects$table_bss_hmc_diagnostics <- creelutils::get_hmc_diagnostics(stan_fit = ecg_fit)
cat(saved_r_objects$table_bss_hmc_diagnostics)

# Export print out
writeLines(
  saved_r_objects$table_bss_hmc_diagnostics,
  file.path(base_path, "diagnostics", "table_bss_hmc_diagnostics.txt")
)

```

```{r model_runtime}
saved_r_objects$table_bss_runtime <- 
  creelutils::get_bss_runtime(
    ecg_fit, 
    output_file = paste0(
      file.path(base_path, "diagnostics"), "/table_bss_runtime.txt")
  )
```
...
#plot traceplots
plot_bss_traceplots <-  mcmc_trace(ecg_fit, pars = c(
    "C_sum", "E_sum", "phi_C_scaled", "phi_E_scaled", "sigma_r_C", "sigma_r_E", "sigma_eps_C", "sigma_eps_E",
    "sigma_mu_C", "sigma_mu_E", "phi_C", "phi_E", "r_C", "r_E", "B1", "lp__"),
    np = nuts_params(ecg_fit)
  ) +
  labs(x = "Post-warmup iteration") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(
  plot = plot_bss_traceplots,
  filename = "plot_bss_traceplots.png",
  path = file.path(base_path, "diagnostics"),
  dpi = 300, width = 7.5, heigh = 6, units = "in", scale = 1.5
)
```
...
# Create catch quantiles
catch_quantiles <- catch_season_results |>
  group_by(est_cg) |>
  reframe(C_sum = quantile(C_sum, c(0.025, 0.5, 0.975)), q = c(0.025, 0.5, 0.975))

# Catch plot
plot_catch <- catch_season_results |>
  ggplot(aes(x = C_sum)) +
  geom_density(fill = "steelblue", alpha = 0.6) +
  geom_vline(data = catch_quantiles, aes(xintercept = C_sum, group = est_cg), linetype = "dashed") +
  ylab(NULL) +
  xlab("Catch (C_sum)") +
  theme_bw() +
...
# Create effort quantiles
effort_quantiles <- effort_season_results |>
  group_by(est_cg) |>
  reframe(E_sum = quantile(E_sum, c(0.025, 0.5, 0.975)), q = c(0.025, 0.5, 0.975))

# Effort plot
plot_effort <- effort_season_results |>
  ggplot(aes(x = E_sum)) +
  geom_density(fill = "steelblue", alpha = 0.6) +
  geom_vline(data = effort_quantiles, aes(xintercept = E_sum, group = est_cg), linetype = "dashed") +
  ylab(NULL) +
  xlab("Effort (E_sum)") +
  theme_bw() +
  theme(legend.position = "none", strip.background = element_blank(), strip.text = element_blank())

plot_bss_probability_densities <- 
  plot_catch / plot_effort +
    plot_layout(guides = "collect") +
    plot_annotation(
      title = "Posterior Densities of Catch and Effort",
      subtitle = glue::glue("Catch group: {catch_group}"),
      tag_levels = 'A'
  ) &
...
ggsave(
  plot = plot_bss_probability_densities,
  filename = "plot_bss_probability_densities.png",
  path = file.path(base_path, "diagnostics"),
  dpi = 300, width = 5, heigh = 3, units = "in", scale = 1.8
)
```
...
saved_r_objects$table_bss_estimates_summary <-
...
write_csv(
  ...
  file = paste0(
    file.path(base_path, "outputs"), 
    "/table_bss_estimates_summary.csv")
)
```
...
ggsave(
  plot_bss_catch_daily,
  filename = "plot_bss_catch_daily.png",
  path = file.path(base_path, "outputs"),
  dpi = 300, width = 7.5, height = 5, scale = 1
)
```
...
ggsave(
  plot_bss_effort_daily,
  filename = "plot_bss_effort_daily.png",
  path = file.path(base_path, "outputs"),
  dpi = 300, width = 7.5, height = 5, scale = 1
)
```
...
ggsave(
  plot_bss_cpue_daily,
  filename = "plot_bss_cpue_daily.png",
  path = file.path(base_path, "outputs"),
  dpi = 300, width = 7.5, height = 5, scale = 1
)
```
...
saved_r_objects$table_estimate_combined_summary <-
  map_df(estimates_bss, ~.x$overview) |> 
    left_join(
      bind_cols(
        estimates_pe$catch |> 
...
#update lut and save out standardized estimates
saved_r_objects$analysis_lut <- analysis_lut
saved_r_objects$creel_estimates  <- creel_estimates

```

```{r drop_draws, eval = TRUE}

# Remove`$draws`from each params$est_catch_groups in estimates_bss
estimates_bss <- purrr::map(estimates_bss, \(x) {
  x$draws <- NULL
  x
})

saved_r_objects$estimates_bss <- estimates_bss
```
...
if (exists("creel_estimates")) {
  ...
  save(saved_r_objects, file = paste0(file.path(base_path), "/saved_r_objects.rda"))
  save(draws_bss, file = paste0(file.path(base_path), "/draws_bss.rds"))
  
} else {
  cli::cli_alert_warning("'creel_estimates.rda' and 'draws_bss.rds' not saved out.")
}

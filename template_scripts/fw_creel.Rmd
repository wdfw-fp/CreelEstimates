...
for(ecg in names(inputs_bss)) {
  ...
  cat("\n")
  cli::cli_rule()
  creelutils::get_hmc_diagnostics(stan_fit = ecg_fit) |> cat()
  ...
  if(length(names(inputs_bss))>1){  
    rm(ecg_fit, ecg_keep, draws_df_full, draws_df, summary_df); gc()
  }
...
# Function to create combined plot for one catch group
plot_catch_effort <- function(cg) {
  # Filter for this catch group
  ...
  # Combine catch and effort vertically with patchwork
...
combined_figure <- wrap_plots(plots_list, ncol = 1) +
  plot_annotation(
    title = "Posterior Densities of Catch and Effort by Catch Group"
  ) &

...
dwg$effort |> 
  ...
  arrange(section_num) |>
  select(`Water Body` = water_body, `Section Number` = section_num, `Location description` = location) |> gt()
```
...
```{r period_dates_reference, eval=FALSE}
...
```{r plot_index_effort_counts, cache=params$enable_cache, dependson="prep_dwg_summ_shared_summary_objects", fig.cap= "Index effort counts for each count sequence on surveyed days within the monitoring period."}
...
### Paired census and index angler effort count plot

```{r view_paired_census_index_counts, fig.cap= "Scatterplot displaying the effort bias-term ratio (TI_expan_final) for each section and angler type (angler_final), representing the total count from census surveys divided by the total count from index surveys. A 1:1 relationship is shown by the dashed line.", results = 'hide', cache=params$enable_cache, dependson=c("establish_file_structure", "prep_inputs_pe")}
...
plot_inputs_pe_census_vs_index(census_TI_expan = inputs_pe$paired_census_index_counts, outputs_folders = outputs_folders)
```
...
for(ecg in names(inputs_bss)) {
  ...
  # Clean up memory - remove large draw objects (unless saving them)
  if (!save_draws) {
    rm(draws_array_temp, draws_df_full, draws_df)
    gc()  # Force garbage collection
  } else {
    rm(draws_df_full)  # Still remove the full unsubsetted version
    gc()
  }
...
```{r plot_prob_density, eval=TRUE, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss")}
...
```{r plot_bss_catch_daily, eval=TRUE, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss")}
...
```{r plot_bss_effort_daily, eval=TRUE, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss")}
...
```{r plot_bss_cpue_daily, eval=TRUE, fig.keep='all', dependson=c("establish_file_structure","estimates_bss")}

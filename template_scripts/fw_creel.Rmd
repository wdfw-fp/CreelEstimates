...
# Variable to toggle model-specific chunks
run_bss <- params$model_used %in% c("BSS only", "Both models")
```
...
```{r qaqc_script, eval=params$run_qaqc}
# When working interactively you might run the script several times
# This prevents unnecessary re-renders of the qaqc script within a given session
check_existing_qaqc <- file.exists(paste0(
  outputs_folders$outputs, "/",
  "qaqc_report_", 
  gsub(" ", "-", params$fishery_name), 
  "_", Sys.Date(), ".html"
))
if (params$run_qaqc && !check_existing_qaqc) {
  # Produce data review summary
  qaqc_result <- creelreview::run_all_checks(dwg)

  # Render QA/QC report in a separate R process
  qaqc_result <- tryCatch({
    callr::r(
      function(fishery_names, output_dir) {
        creelreview::generate_report(
          fishery_names = fishery_names,
          output_dir = output_dir
        )
      },
      args = list(
        fishery_names = params$fishery_name,
        output_dir = outputs_folders$qaqc
      ),
      timeout = 600
    )
  }, error = function(e) {
    warning("QA/QC report render failed: ", conditionMessage(e))
    data.frame(
  ...
  cat("\nQA/QC report saved to: \n", outputs_folders$outputs)
}

#Print summary
if (!is.null(qaqc_result)) print(qaqc_result)
```
...
```{r prep_inputs_bss, eval=run_bss, cache=params$enable_cache, dependson="prep_dwg_summ_shared_summary_objects", cache.extra=param_hash}
...
```{r estimates_bss, eval=run_bss, results='hide', fig.show='hide', cache=params$enable_cache, dependson=c("establish_file_structure", "prep_inputs_bss"), cache.extra=param_hash}
...
```{r summary_convergence_diagnostics, eval=run_bss, results='asis'}
...
```{r bss_trace_plots, eval=run_bss, fig.keep='all', dependson="estimates_bss"}
...
```{r plot_bss_effort_daily, eval=run_bss, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss"), fig.cap= "Weekly estimates of angler effort (angler hours) during the fishery, with separate plots for each river section. Lines are posterior medians and shading shows 95% credible intervals and are colored by angler type."}
...
```{r ppc_interval_coverage, eval=run_bss}
...
```{r ppc_plots_display, eval=run_bss, fig.height=8, fig.width=10, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss")}
...
```{r estimates_combo_overview, eval=run_bss, dependson=c("estimates_bss, estimates_pe")}
...
```{r effort_sampled_by_interviews, cache=params$enable_cache, dependson=c("dwg_fetch", "prep_dwg_summ_shared_summary_objects", "estimates_pe"), cache.extra=param_hash}

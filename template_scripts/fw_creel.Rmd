...
params <- yaml::read_yaml(file = here(file.path("input_files/_params.yml")))

# Rebuild old YAML catch group format for backwards compatibility
params$est_catch_groups <- data.frame(
  species = params$species,
  life_stage = params$life_stage,
  fin_mark = params$fin_mark,
  fate = params$fate,
...
script_path <- here(
  ...
  paste0("script_", params$fishery_name, ".Rmd")
)

if (file.exists(script_path)) {
  cli_alert_warning("An analysis script for this fishery already exists at {.path {script_path}}")
} else {
...
# remove closures in section 2 now that it has been aggregated with section 1
...
get_hmc_diagnostics_summary <- function(stan_fit = ecg_fit) {
...
saved_r_objects$table_bss_hmc_diagnostics <- get_hmc_diagnostics_summary()

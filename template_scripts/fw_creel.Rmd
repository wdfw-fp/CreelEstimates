...
params:
  ...
  bss_n_chain: 2
  bss_n_cores: 2
  bss_n_iter: 50      # NOTE: dev-scale default; production runs should set this much higher
  bss_n_warmup: 25    # NOTE: dev-scale default; production runs should set this much higher
  bss_n_thin: 1
  bss_adapt_delta: 0.7
  bss_max_treedepth: 12
  bss_init: "0"
  bss_save_draws: FALSE  # TRUE saves posterior draw objects (WARNING: large file sizes)
  bss_priors: !r c(
  ...
  bss_empirical_mu_mu_C: FALSE  # TRUE re-centers the mu_C hyperprior mean at log(observed CPUE), per catch group, overriding value_normal_mu_mu_C in bss_priors
...
inputs_bss <-
  ...
  map(
    ~ {
      ecg <- .x
      # unlist() so both the header default (named numeric vector) and a
      # YAML-map override from an automation config (named list) work
      priors_ecg <- unlist(params$bss_priors)

      # Optional empirical Bayes seeding: center the mu_C hyperprior mean on
      # the observed season CPUE for this catch group. Uses the same
      # interview rows (zero-filled catch, group fishing hours) that become
      # the Stan c/h vectors. Falls back to the bss_priors value when
      # observed CPUE is zero/undefined (log(0) would be -Inf).
      if (isTRUE(params$bss_empirical_mu_mu_C)) {
        obs_cpue <- dwg_summ$interview |>
          filter(est_cg == ecg) |>
          summarise(cpue = sum(fish_count, na.rm = TRUE) / sum(fishing_time_total, na.rm = TRUE)) |>
          pull(cpue)

        if (is.finite(obs_cpue) && obs_cpue > 0) {
          priors_ecg[["value_normal_mu_mu_C"]] <- log(obs_cpue)
          cli_alert_info("{ecg}: value_normal_mu_mu_C set empirically to log({signif(obs_cpue, 4)}) = {signif(log(obs_cpue), 4)}")
        } else {
          cli_alert_warning("{ecg}: observed CPUE is zero or undefined; keeping value_normal_mu_mu_C = {priors_ecg[['value_normal_mu_mu_C']]} from bss_priors")
        }
      }
...
# ---- CONFIGURATION ----
# Controlled by params$bss_save_draws (WARNING: TRUE means large file sizes)
save_draws <- isTRUE(params$bss_save_draws)
# ----------------------
...
for(ecg in names(inputs_bss)) {
  ...
  ecg_fit <-
    fit_bss(
      ...
      n_chain = params$bss_n_chain,
      n_cores = params$bss_n_cores,
      n_iter = params$bss_n_iter,
      n_warmup = params$bss_n_warmup,
      n_thin = params$bss_n_thin,
      adapt_delta = params$bss_adapt_delta,
      max_treedepth = params$bss_max_treedepth,
      init = params$bss_init
    )

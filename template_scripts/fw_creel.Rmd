...
params:
  ...
  fishery_name: "Skagit summer sockeye 2026"
  est_date_start: "2026-06-16"
  est_date_end: "2026-06-24"
  est_catch_groups: !r data.frame(rbind(
    c(species = 'Sockeye', life_stage = 'Adult', fin_mark = 'AD', fate = 'Kept'),
    c(species = 'Sockeye', life_stage = 'Adult', fin_mark = 'UM', fate = 'Kept'),
    c(species = 'Sockeye', life_stage = 'Adult', fin_mark = 'AD', fate = 'Released'),
    c(species = 'Chinook', life_stage = 'Adult', fin_mark = 'AD', fate = 'Released'),
    c(species = 'Bull Trout', life_stage = 'NA', fin_mark = 'NA', fate = 'Released'),
    c(species = 'Dolly Varden', life_stage = 'NA', fin_mark = 'NA', fate = 'Released')))
  ...
  bss_model_file_name: "BSS_creel_model_02_2026-01-15.stan"
...
inputs_bss <- 
  ...
  map(
    ~prep_inputs_bss(
      ...
      study_design = params$study_design, 
      priors = c(
# Half-Student-t regularizing priors. Adjust degrees of freedom (df) to control tail geometry and adjust scale to change SD. Larger scale settings (>0.5) keep priors mildly informative and mostly affect data-poor strata. Larger DF settings shrink tails:
  # df = 1   → Cauchy (very heavy tails) - extreme robustness, weaker regularization needs small scale when data are sparse
  # df = 3   → Heavy tails - strong robustness
  # df = 4   → Moderate tails - recommended for variance parameters
  # df = 20   → Light tails - close to normal, stronger regularization
  # df ≥ 30  → Nearly normal - lightest tails
  # Catch process error (sigma_eps_C) - CPUE daily fluctuations
        value_student_t_df_sigma_eps_C = 30,       
        value_student_t_scale_sigma_eps_C = 0.75,  
   # Effort process error (sigma_eps_E) - effort daily fluctuations
        value_student_t_df_sigma_eps_E = 30,
        value_student_t_scale_sigma_eps_E = 0.75,
   # Effort overdispersion (sigma_r_E) - between-angler effort variation 
        value_student_t_df_sigma_r_E = 30,         
        value_student_t_scale_sigma_r_E = 0.75,
   # Catch overdispersion (sigma_r_C) - between-angler CPUE variation
        value_student_t_df_sigma_r_C = 30,
        value_student_t_scale_sigma_r_C = 0.75,
   # Catch spatial heterogeneity (sigma_mu_C) - between-section CPUE variation 
        value_student_t_df_sigma_mu_C = 30,
        value_student_t_scale_sigma_mu_C = 0.75,
   # Effort spatial heterogeneity (sigma_mu_E) - between-section effort variation
        value_student_t_df_sigma_mu_E = 30,
        value_student_t_scale_sigma_mu_E = 0.75, 
# Other priors:
        value_normal_sigma_omega_C_0 = 1, # Initial state uncertainty
        value_normal_sigma_omega_E_0 = 3, # Initial state uncertainty
        value_lognormal_sigma_b = 1, # Overdispersion 
        value_normal_sigma_B1 = 5, # Overdispersion
        value_normal_mu_mu_C = log(
          sum(dwg_summ$interview$fish_count)/sum(dwg_summ$interview$fishing_time_total)
        ), # Mean CPUE location. Empirical Bayes. Set from raw data
        value_normal_sigma_mu_C = 1.5, # Mean CPUE SD
        value_normal_mu_mu_E = log(5), # Median anglers/hr per gear type
        value_normal_sigma_mu_E = 2, # Median effort SD
        value_betashape_phi_E_scaled = 1, # Autocorrelation coefficient for effort
        value_betashape_phi_C_scaled = 1 # Autocorrelation coefficient for catch
    )
    )
  )
...
for(ecg in names(inputs_bss)) {
  ...
  # create prior-posterior plot inputs
  ecg_keep$priorposteriorplots <- create_prior_posterior_plots(
    draws_df = draws_df,
    stan_data = inputs_bss[[ecg]],
    ecg = ecg
  )
  estimates_bss[[ecg]] <- ecg_keep
...
overview_combined |> 
  ...
  fmt_number(-c(estimate, est_cg, catch_group, Rhat), decimals = 2) |> 
...
#### Prior-Posterior Plots for Variance Priors

```{r priorposteriorplots, eval=TRUE, echo=FALSE, fig.keep='all', dependson=c("estimates_bss")}
for (ecg in names(estimates_bss)) {
  
  pp <- estimates_bss[[ecg]]$priorposteriorplots
  if (is.null(pp) || is.null(pp$plots_list) || length(pp$plots_list) == 0) next
  
  combined_figure <-
    wrap_plots(pp$plots_list, ncol = 2) +
    plot_annotation(
      title    = "Prior vs Posterior vs Likelihood — Variance Parameters",
      subtitle = glue::glue("Catch group: {ecg}"),
      theme    = theme(
  ...
  save_plot(
    combined_figure,
    paste0("prior_posterior_likelihood_variance_", pp$safe_name, ".png"),
    width = 12,
    height = 10
  )
}
...
# Dynamic sizing
n_groups <- length(plots_list)
height_per_group <- 3
fig_width  <- 10
fig_height <- max(8, height_per_group * n_groups)
...
```{r ppc_plots_display, eval=TRUE, fig.height=3, fig.width=10, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss")}
...
purrr::iwalk(estimates_bss, ~{
  ...
  purrr::iwalk(.x$ppc_results$plots, \(plt, plt_name) {
    if (!is.null(plt)) {
      print(plt)
      save_plot(plt,
                paste0("ppc_", stringr::str_remove(plt_name, "_plt$"), 
                       "_", safe_catch_group, ".png"),
                width = 10, height = 8)
    }
  })
})

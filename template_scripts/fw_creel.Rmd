...
params:
  ...
  fishery_name: "Skagit winter steelhead 2025"
  est_date_start: "2025-02-15"
  est_date_end: "2025-03-01"
  est_catch_groups: !r data.frame(rbind(
    c(species = 'Steelhead', life_stage = 'Adult', fin_mark = 'UM', fate = 'Released'),
    c(species = 'Bull Trout|Dolly Varden', life_stage = 'Adult|NA|UNK', fin_mark = 'UM|UNK|NA', fate = 'Released')
    ))
...
knitr::opts_chunk$set(
  warning = FALSE,
  message = FALSE,
  cache = FALSE,          # Will be set to TRUE by setup_analysis_structure
  fig.width = 10,
  fig.height = 8
)
...
## Download data
```{r dwg_fetch, cache=TRUE, results='hide', dependson="setup", cache.extra=param_hash}
...
```{r prep_dwg_summ_shared_summary_objects, results = 'hide', cache=TRUE, dependson="dwg_fetch", cache.extra=param_hash}
...
```{r plot_index_effort_counts, cache=TRUE, dependson="prep_dwg_summ_shared_summary_objects", fig.cap= "Index effort counts for each count sequence on surveyed days within the monitoring period."}
...
```{r prep_inputs_pe, results = 'hide', cache=TRUE, dependson="prep_dwg_summ_shared_summary_objects", cache.extra=param_hash}
...
```{r estimates_pe, cache=TRUE, dependson="prep_inputs_pe", cache.extra=param_hash}
estimates_pe <- list() 
...
```{r plot_pe_effort_estimates, fig.cap= "Estimated effort (angler-hours) grouped by time strata, section and angler type (angler_final).", cache=TRUE, dependson=c("establish_file_structure", "estimates_pe"), cache.extra=param_hash}
...
```{r pe_cpue_period_plots, results = 'hide',  cache=TRUE, dependson= c("prep_inputs_pe","dwg_fetch"), cache.extra=param_hash}
...
```{r plot_pe_catch_estimates, results = 'hide', cache=TRUE, dependson="estimates_pe", cache.extra=param_hash}
...
```{r prep_inputs_bss, eval=TRUE, cache=TRUE, dependson="prep_dwg_summ_shared_summary_objects", cache.extra=param_hash}
...
```{r estimates_bss, eval=TRUE, results='hide', fig.show='hide', cache=TRUE, dependson=c("establish_file_structure", "prep_inputs_bss"), cache.extra=param_hash}
...
```{r transform_estimates, eval = TRUE, results='hide', cache=TRUE, dependson="estimates_bss", cache.extra=param_hash}

...
for(ecg in names(inputs_bss)) {
  ...
  #CH 7/29/25 extract draws, trim, and transform before removing stanfit object when >1 catch groups
  ...
  #get bss diagnostics using creelutils package
  bss_runtime_cg <- creelutils::get_bss_runtime(ecg_fit)
  cat("\n")
  cli::cli_rule()
  bss_hmc_diagnostic_cg <- creelutils::get_hmc_diagnostics(stan_fit = ecg_fit) |> cat() |> capture.output()
  cat(bss_hmc_diagnostic_cg, sep = "\n")
  cat("\n")
  cli::cli_rule()
  bss_settings_cg <- get_bss_settings(ecg, ecg_fit, inputs_bss, print = TRUE) |> capture.output()
  cat(bss_settings_cg, sep = "\n")
  
  
  # store iteration
  estimates_bss[[ecg]] <- ecg_keep
  
  bss_settings[[ecg]] <- list(
    runtime = bss_runtime_cg,
    sampling_diagnostics = bss_hmc_diagnostic_cg,
    model_settings = bss_settings_cg
  )

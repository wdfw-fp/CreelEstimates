...
# Package settings (cmdstanr caches compiled models automatically)
...
## Download data
```{r dwg_fetch, cache=params$enable_cache, results='hide', cache.extra=param_hash}
...
```{r view_paired_census_index_counts, cache=params$enable_cache, dependson="prep_inputs_pe"}
...
```{r plot_pe_effort_estimates, fig.cap= "Estimated effort (angler-hours) grouped by time strata, section and angler type (angler_final).", cache=params$enable_cache, dependson="estimates_pe", cache.extra=param_hash}
...
```{r estimates_bss, eval=TRUE, results='hide', fig.show='hide', cache=params$enable_cache, dependson="prep_inputs_bss", cache.extra=param_hash}
...
# ----------------------
compile_bss_model
# ---- Compile / load model once (shared across all catch groups) ----
# Compiles only when no exe is found; subsequent runs load the existing exe.
# Default stan_exe_dir = dirname(cmdstan_path()) = C:\Users\<user>\.cmdstan\
# (user-agnostic; exe persists across CmdStan version upgrades)
bss_model <- compile_bss_model(
  model_file_name   = here::here("stan_models", params$bss_model_file_name),
  threads_per_chain = 2
)
...
for(ecg in names(inputs_bss)) {
  ...
  ecg_fit <-
    fit_bss(
      bss_model_obj     = bss_model,      # pre-compiled; no recompilation per catch group
      ...
      adapt_delta       = 0.7,
      max_treedepth     = 12,
      threads_per_chain = 2,              # within-chain parallelization via reduce_sum (total CPU = n_cores * threads_per_chain)
      init              = "0"
    )
  ...
  # Always extract draws for use in diagnostics and plots (cmdstanr)
  ecg_keep$draws <- list(
    C_sum = as.vector(ecg_fit$draws(variables = "C_sum", format = "matrix")),
    E_sum = as.vector(ecg_fit$draws(variables = "E_sum", format = "matrix"))
  )
  ...
  ecg_keep$summary <- ecg_fit$summary()
  ...
  draws_array_temp <- ecg_fit$draws(format = "draws_array")
  ...
  # TODO: Update creelutils package (wdfw-fp/creelutils) to support cmdstanr fit objects
  # get_bss_runtime fallback
  diagnostics_keep$bss_runtime_cg <- tryCatch(
    {
      creelutils::get_bss_runtime(ecg_fit)
    },
    error = function(e) {
      warning(paste0(
        "creelutils::get_bss_runtime() may not be compatible with cmdstanr fit objects. ",
        "Using local fallback. Error: ", e$message
      ))
      ecg_fit$time()
    }
  )
  ...
  # get_hmc_diagnostics fallback
  diagnostics_keep$bss_hmc_diagnostic_cg <- tryCatch(
    {
      capture.output(creelutils::get_hmc_diagnostics(stan_fit = ecg_fit))
    },
    error = function(e) {
      warning(paste0(
        "creelutils::get_hmc_diagnostics() may not be compatible with cmdstanr fit objects. ",
        "Using local fallback. Error: ", e$message
      ))
      capture.output(ecg_fit$diagnostic_summary())
    }
  )
  ...
  # get_bss_settings fallback
  diagnostics_keep$bss_settings_cg <- tryCatch(
    {
      get_bss_settings(ecg, ecg_fit, inputs_bss, print = TRUE) |> capture.output()
    },
    error = function(e) {
      warning(paste0(
        "get_bss_settings() may not be compatible with cmdstanr fit objects. ",
        "Using local fallback. Error: ", e$message
      ))
      capture.output(cat(paste0(
        "Catch group: ", ecg, "\n",
        "Chains: ", ecg_fit$num_chains(), "\n",
        "Iter sampling: ", ecg_fit$metadata()$iter_sampling, "\n",
        "Iter warmup: ", ecg_fit$metadata()$iter_warmup, "\n",
        "Thin: ", ecg_fit$metadata()$thin, "\n",
        "Adapt delta: ", ecg_fit$metadata()$adapt_delta, "\n",
        "Max treedepth: ", ecg_fit$metadata()$max_treedepth, "\n"
      )))
    }
  )
  cat(diagnostics_keep$bss_settings_cg, sep = "\n")

  # trace plots — cmdstanr: pass explicit draws array + sampler diagnostics to bayesplot
  np_data <- tryCatch(
    nuts_params(ecg_fit$sampler_diagnostics(format = "draws_array")),
    error = function(e) NULL
  )
  invisible(diagnostics_keep$traceplots <- mcmc_trace(
    ecg_fit$draws(format = "draws_array"), 
    pars = c(
...
# Select and order columns: overview uses rstan-compatible names (estimate, Rhat, n_eff)
overview_combined |>
  select(catch_group, estimate, est_cg, mean, sd,
         `2.5%`, `25%`, `50%`, `75%`, `97.5%`,
         Rhat, n_eff, n_div) |>
  gt(groupname_col = "catch_group") |>
  fmt_number(columns = c(mean, sd, `2.5%`, `25%`, `50%`, `75%`, `97.5%`), decimals = 1) |>
  fmt_number(columns = Rhat, decimals = 3) |>
  fmt_number(columns = c(n_eff, n_div), decimals = 0) |>
  tab_style(
    style = cell_fill(),
...
```{r plot_bss_catch_daily, eval=TRUE, fig.keep='all', dependson="estimates_bss", fig.cap= "Weekly estimates of catch for catch groups included in the report, with separate plots for each river section. Lines are posterior medians and shading shows 95% credible intervals and are colored by angler type."}
...
```{r plot_bss_effort_daily, eval=TRUE, fig.keep='all', dependson="estimates_bss", fig.cap= "Weekly estimates of angler effort (angler hours) during the fishery, with separate plots for each river section. Lines are posterior medians and shading shows 95% credible intervals and are colored by angler type."}
...
```{r plot_bss_cpue_daily, eval=TRUE, fig.keep='all', dependson="estimates_bss", fig.cap= "Weekly estimates of catch rate (fish/hour) for catch groups included in the report, with separate plots for each river section. Lines are posterior medians and shading shows 95% credible intervals and are colored by angler type."}

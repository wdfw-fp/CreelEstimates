...
params:
  ...
  fishery_name: "Cascade spring Chinook 2022"
...
#KB NOTES: the following BSS summary output does not appear to work (I think there is an issue with the cg name)
...
#KB NOTES: this doesn't work because "bss_overview" doesn't work above
...
if (params$export %in% c("database", "local") && !params$model_used %in% c("PE only", "BSS only", "Both models" )) {
  cat("Invalid value for model_used parameter. Must select 'PE only', 'BSS only', or 'Both models'.")
} else {

  # Initialize objects for storing processed data
  # also serves as placeholder for other model if only one was used
  ...
  if (params$model_used == "PE only") {
    export_estimates(params, estimates_pe, estimates_bss = NULL)
  }
  
  if (params$model_used == "BSS only") {
    export_estimates(params, estimates_pe = NULL, estimates_bss)
  }
  
  if (params$model_used == "Both models") {
    export_estimates(params, estimates_pe, estimates_bss)
  }
}

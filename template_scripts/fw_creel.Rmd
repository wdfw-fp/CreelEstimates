...
reg_shading_catch <- catch_daily_df |>
  ...
  summarise(xmin = min(event_date), xmax = max(event_date), .groups = "drop")

plot_bss_catch_daily <-
  catch_daily_df |> 
    ggplot(aes(x = event_date, y = `50%`)) +
    geom_rect(
    ...
    labs(color = "Angler type", fill = "Angler type",
         caption = "Light blue shading = boat fishing open") +
...
save_plot(
  ...
  "bss_catch_daily.png",
  width = 12,
  height = 4 + (2 * n_distinct(catch_daily_df$est_cg))
)
```
...
effort_by_reg <- map_df(estimates_bss, ~.x$effort_daily) |>
  ...
  summarise(
    n_days = n(),
    total_effort_median = sum(`50%`),
    mean_daily_effort_median = mean(`50%`),
    .groups = "drop"
  )
...
catch_by_reg <- map_df(estimates_bss, ~.x$catch_daily) |>
  ...
  summarise(
    n_days = n(),
    total_catch_median = sum(`50%`),
    mean_daily_catch_median = mean(`50%`),
    .groups = "drop"
  )
...
reg_summary <- effort_by_reg |>
  left_join(
    catch_by_reg |> select(est_cg, section_num, angler_final, boat_reg_label, 
                           total_catch_median, mean_daily_catch_median),
    by = c("est_cg", "section_num", "angler_final", "boat_reg_label")
  ) |>
  mutate(
    mean_daily_cpue = mean_daily_catch_median / mean_daily_effort_median
  )

reg_summary_table <- reg_summary |>
  gt(groupname_col = "est_cg") |>
  fmt_number(columns = c(total_effort_median, mean_daily_effort_median, 
                         total_catch_median, mean_daily_catch_median, mean_daily_cpue), 
             decimals = 1) |>
  cols_label(
    section_num = "Section",
    ...
    n_days = "Days",
    total_effort_median = "Total effort (hrs)",
    mean_daily_effort_median = "Mean daily effort (hrs)",
    total_catch_median = "Total catch",
    mean_daily_catch_median = "Mean daily catch",
    mean_daily_cpue = "Mean CPUE (fish/hr)"
  ) |>
  tab_header(
    title = "Effort and catch estimates grouped by boat regulation status",
    subtitle = "Summaries of posterior medians by section, angler type, and regulation period"
  ) |>
  tab_options(
    heading.title.font.size = px(14),
    table.font.color = "black"

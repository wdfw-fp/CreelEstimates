...
params:
  ...
  bss_model_file_name: "BSS_creel_model_02_2021-01-22_ppc_withboateffect.stan"
...
# ---- Build the regime day_groups object ----
# Example: Hoh River 2023-24 boat regulation (Table 1 of the study design),
# where boat fishing allowance varies by section AND day of week. Replace
# this case_when with the actual regulation structure for the fishery in
# this report; the shape required is a data frame with one row per
# event_date x section_num (or just event_date for a uniform regulation)
# and a single grouping column (here, boat_allowed).
...
hoh_boat_reg <- dwg$days |>
  ...
  dplyr::select(-day) |>
  dplyr::arrange(event_date, section_num) |>
  tidyr::pivot_wider(
    id_cols = event_date,
    names_from = section_num,
    names_prefix = "open_section_",
    values_from = boat_allowed
  ) |>
  dplyr::arrange(event_date) |>
  dplyr::select(-event_date)
# }

# Create a long-format regulation lookup for joining to daily results
# Maps day_index × section_num → boat_reg (open/closed)
regulation_lookup <- hoh_boat_reg |>
  mutate(day_index = row_number()) |>
  pivot_longer(
    cols = starts_with("open_section_"),
  ...
  mutate(
    ...
    boat_reg_label = if_else(boat_reg, "Boat fishing open", "Boat fishing closed")
  ) |>
  select(day_index, section_num, boat_reg, boat_reg_label)
...
for(ecg in names(inputs_bss)) {
  ...
  ecg_keep$cpue_daily <- get_bss_cpue_daily(bss_fit = ecg_fit, ecg = ecg, dwg = dwg) |>
    left_join(regulation_lookup, by = c("day_index", "section_num"))
  ...
  # Extract B2 (boat regulation effect on effort, log-scale)
  ecg_keep$B2 <- ecg_fit |>
    summary(pars = "B2") |>
    pluck("summary") |>
    as.data.frame() |>
    ...
    as_tibble() |>
    mutate(
      indices = str_extract(parameter, "\\d+,\\d+"),
      gear = as.integer(str_extract(indices, "^\\d+")),
      section_num = as.integer(str_extract(indices, "\\d+$")),
      angler_final = if_else(gear == 1, "bank", "boat"),
      est_cg = ecg
    ) |>
    select(est_cg, angler_final, section_num, mean, sd, `2.5%`, `50%`, `97.5%`, n_eff, Rhat)
...
```{r plot_bss_catch_daily, eval=TRUE, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss"), fig.cap= "Weekly estimates of catch for catch groups included in the report, with separate plots for each river section. Lines are posterior means and shading shows 95% credible intervals and are colored by angler type."}

catch_daily_df <- map_df(estimates_bss, ~.x$catch_daily)

# Build regulation shading rectangles for background
reg_shading_catch <- catch_daily_df |>
  distinct(section_num, event_date, boat_reg_label) |>
  arrange(section_num, event_date) |>
  group_by(section_num) |>
  mutate(reg_change = boat_reg_label != lag(boat_reg_label, default = first(boat_reg_label))) |>
  mutate(reg_group = cumsum(reg_change)) |>
  group_by(section_num, reg_group, boat_reg_label) |>
  summarise(xmin = min(event_date), xmax = max(event_date) + 1, .groups = "drop")

plot_bss_catch_daily <-
  catch_daily_df |> 
    ggplot(aes(x = event_date, y = mean)) +
    geom_rect(
      data = reg_shading_catch |> filter(boat_reg_label == "Boat fishing open"),
      aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf),
      fill = "#FFF9C4", alpha = 0.6, inherit.aes = FALSE
    ) +
    geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`, fill = angler_final), alpha = 0.2, color = "transparent") +
    geom_line(aes(color = angler_final), lwd = 1) +
    ylab("Mean catch") +
    xlab("Date") +
    labs(color = "Angler type", fill = "Angler type",
         caption = "Yellow background shading = boat fishing open (regulation covariate); unshaded = boat fishing closed.\nRibbon shading (colored by angler type) = 95% credible interval.") +
    scale_x_date(date_breaks = "1 week", date_labels = "%m-%d") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8)) +
    facet_grid(section_num ~ est_cg, scales = "free_y")
...
save_plot(
  ...
  "bss_catch_daily.png",
  width = 12,
  height = 4 + (2 * n_distinct(catch_daily_df$section_num))
)
```

#### plot daily effort BSS

```{r plot_bss_effort_daily, eval=TRUE, fig.keep='all', dependson=c("establish_file_structure", "estimates_bss"), fig.cap= "Weekly estimates of angler effort (angler hours) during the fishery, with separate plots for each river section. Lines are posterior means; ribbon shading (colored by angler type) shows 95% credible intervals. Yellow background shading indicates periods when boat fishing was open; unshaded periods indicate boat fishing was closed."}

effort_daily_df <- map_df(estimates_bss, ~.x$effort_daily)

reg_shading_effort <- effort_daily_df |>
  distinct(section_num, event_date, boat_reg_label) |>
  arrange(section_num, event_date) |>
  group_by(section_num) |>
  mutate(reg_change = boat_reg_label != lag(boat_reg_label, default = first(boat_reg_label))) |>
...
plot_bss_effort_daily <-
  effort_daily_df |> 
    ggplot(aes(x = event_date, y = mean)) +
    geom_rect(
      data = reg_shading_effort |> filter(boat_reg_label == "Boat fishing open"),
      aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf),
      fill = "#FFF9C4", alpha = 0.6, inherit.aes = FALSE
    ) +
    geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`, fill = angler_final), alpha = 0.2, color = "transparent") +
    geom_line(aes(color = angler_final), lwd = 1) +
    ylab("Mean effort (angler hours)") +
    xlab("Date") +
    labs(color = "Angler type", fill = "Angler type",
         caption = "Yellow background shading = boat fishing open (regulation covariate); unshaded = boat fishing closed.\nRibbon shading (colored by angler type) = 95% credible interval.") +
    scale_x_date(date_breaks = "1 week", date_labels = "%m-%d") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8)) +
    facet_wrap(~section_num, nrow = n_distinct(effort_daily_df$section_num))
...
cpue_daily_df <- map_df(estimates_bss, ~.x$cpue_daily)

reg_shading_cpue <- cpue_daily_df |>
  distinct(section_num, event_date, boat_reg_label) |>
  arrange(section_num, event_date) |>
  group_by(section_num) |>
  mutate(reg_change = boat_reg_label != lag(boat_reg_label, default = first(boat_reg_label))) |>
...
plot_bss_cpue_daily <-
  cpue_daily_df |> 
    ggplot(aes(x = event_date, y = mean)) +
    geom_rect(
      data = reg_shading_cpue |> filter(boat_reg_label == "Boat fishing open"),
      aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf),
      fill = "#FFF9C4", alpha = 0.6, inherit.aes = FALSE
    ) +
    geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`, fill = angler_final), alpha = 0.2, color = "transparent") +
    geom_line(aes(color = angler_final), lwd = 1) +
    ylab("Mean catch per unit effort (fish/hour)") +
    xlab("Date") +
    labs(color = "Angler type", fill = "Angler type",
         caption = "Yellow background shading = boat fishing open (regulation covariate); unshaded = boat fishing closed.\nRibbon shading (colored by angler type) = 95% credible interval.") +
    scale_x_date(date_breaks = "1 week", date_labels = "%m-%d") +
...
save_plot(
  ...
  "bss_cpue_daily.png",
  width = 12,
  height = 4 + (2 * n_distinct(cpue_daily_df$section_num))
)
```

### BSS estimates by boat regulation status

#### B2 parameter: estimated boat regulation effect on effort

The `B2` parameter represents the effect of boat fishing regulations on angler effort (on the log scale). Values > 0 indicate higher effort when boat fishing is open; values < 0 indicate lower effort. The multiplicative effect on effort is `exp(B2)`.

```{r B2_regulation_effect, eval=TRUE, dependson="estimates_bss"}
# Extract and display B2 parameter estimates
B2_combined <- purrr::map_df(estimates_bss, ~.x$B2)

B2_combined |>
  mutate(
    mult_effect = round(exp(`50%`), 2),
    mult_lower = round(exp(`2.5%`), 2),
    mult_upper = round(exp(`97.5%`), 2)
  ) |>
  select(est_cg, angler_final, section_num, 
         `B2 median (log)` = `50%`, 
         `Effort multiplier` = mult_effect,
         `2.5%` = mult_lower, `97.5%` = mult_upper,
         Rhat) |>
  gt(groupname_col = "est_cg") |>
  fmt_number(columns = c(`B2 median (log)`), decimals = 3) |>
  fmt_number(columns = c(`Effort multiplier`, `2.5%`, `97.5%`), decimals = 2) |>
  fmt_number(columns = Rhat, decimals = 3) |>
  tab_header(
    title = "Boat regulation effect (B2) on angler effort",
    subtitle = "Effort multiplier = exp(B2): values > 1 indicate increased effort when boat fishing is open"
  ) |>
  tab_options(
    heading.title.font.size = px(14),
    table.font.color = "black"
...
#### Summary of effort and catch grouped by regulation status

```{r regulation_grouped_summary, eval=TRUE, dependson="estimates_bss"}

# Effort grouped by regulation status
effort_by_reg <- map_df(estimates_bss, ~.x$effort_daily) |>
  group_by(est_cg, section_num, angler_final, boat_reg_label) |>
  summarise(
    n_days = n(),
    total_effort_mean = sum(mean),
    mean_daily_effort_mean = mean(mean),
    .groups = "drop"
  )

# Catch grouped by regulation status
catch_by_reg <- map_df(estimates_bss, ~.x$catch_daily) |>
  group_by(est_cg, section_num, angler_final, boat_reg_label) |>
  summarise(
    n_days = n(),
    total_catch_mean = sum(mean),
    mean_daily_catch_mean = mean(mean),
    .groups = "drop"
  )

# Combined regulation summary table
reg_summary <- effort_by_reg |>
  left_join(
    catch_by_reg |> select(est_cg, section_num, angler_final, boat_reg_label, 
                           total_catch_mean, mean_daily_catch_mean),
    by = c("est_cg", "section_num", "angler_final", "boat_reg_label")
  ) |>
  mutate(
    mean_daily_cpue = mean_daily_catch_mean / mean_daily_effort_mean
  )

reg_summary_table <- reg_summary |>
  gt(groupname_col = "est_cg") |>
  fmt_number(columns = c(mean_daily_cpue), 
             decimals = 2) |>
  fmt_number(columns = c(total_effort_mean, mean_daily_effort_mean, 
                         total_catch_mean, mean_daily_catch_mean), 
             decimals = 1) |>
  cols_label(
    section_num = "Section",
    angler_final = "Angler type",
    boat_reg_label = "Regulation",
    n_days = "Days",
    total_effort_mean = "Total effort (hrs)",
    mean_daily_effort_mean = "Mean daily effort (hrs)",
    total_catch_mean = "Total catch",
    mean_daily_catch_mean = "Mean daily catch",
    mean_daily_cpue = "Mean CPUE (fish/hr)"
  ) |>
  tab_header(
    title = "Effort and catch estimates grouped by boat regulation status",
    subtitle = "Summaries of posterior means by section, angler type, and regulation period"
  ) |>
  tab_options(
    heading.title.font.size = px(14),
    table.font.color = "black"
...
# Save to figures folder
save_gt_table(reg_summary_table, "regulation_grouped_summary")
```
...
```{r transform_estimates, eval = FALSE, results='hide', cache=params$enable_cache, dependson="estimates_bss", cache.extra=param_hash}
#Extract and transform model estimates into a common format
transformed_pe_data <- list()
transformed_bss_data <- list()

# Create a clean copy of estimates_bss without regulation columns for creelutils export
# (creelutils expects the standard column layout for rbind with PE estimates)
estimates_bss_clean <- map(estimates_bss, function(ecg_list) {
  reg_cols <- c("boat_reg", "boat_reg_label")
  ecg_list$effort_daily <- ecg_list$effort_daily |> select(-any_of(reg_cols))
  ecg_list$catch_daily  <- ecg_list$catch_daily  |> select(-any_of(reg_cols))
  ecg_list$cpue_daily   <- ecg_list$cpue_daily   |> select(-any_of(reg_cols))
  ecg_list
})
...
if (params$export %in% c("database", "local") && !params$model_used %in% c("PE only", "BSS only", "Both models" )) {
  ...
  if (exists("estimates_bss") && params$model_used == "BSS only") {
    ...
    transformed_bss_data <- creelutils::process_estimates_bss(params, analysis_lut, estimates_bss_clean)
  ...
  if (exists("estimates_bss") && exists("estimates_pe") && params$model_used == "Both models") {
...
```{r export_estimates, results='hide', eval= FALSE}

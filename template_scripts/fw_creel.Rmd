...
params:
  ...
  fishery_name: "Skagit winter steelhead 2024"
  est_date_start: "2024-02-01"
  est_date_end: "2024-04-14"
  est_catch_groups: !r data.frame(rbind(
    c(species = 'Steelhead', life_stage = 'Adult', fin_mark = 'UM|AD', fate = 'Released')
    ))
...
<<<<<<< HEAD
  ...
  export: "local"           #Default: local             options:  "no", "local", "database".
  export_tables: "both"     #Default: total             options: "total", "stratum", "both".
  model_used: "PE only"    #Default: PE only           options: "PE only", "BSS only", "Both models".
  data_grade: "provisional" #Default: provisional       options: "provisional", "approved".
=======
  output_location_filepath: "local" #can remove param with new ETL
  #ETL parameters
  export: "local" #options:  "no", "local", "database". -Default: local
  export_tables: "both" #options: "total", "stratum", "both". -Default: total
  model_used: "PE only" #options: "PE only", "BSS only", "Both models". -Default: PE only
  data_grade: "provisional" #options: "provisional", "approved". Note: approved data may only be submitted by FRFM admin
>>>>>>> parent of 0e83b90 (Merge branch 'estimate_storage' of https://github.com/wdfw-fp/CreelEstimates into estimate_storage)
output:
  html_document:
    fig_caption: yes
    theme: default
...
<<<<<<< HEAD
```{r transform_estimates, eval = FALSE}
...
if (params$export %in% c("database", "local") && !params$model_used %in% c("PE only", "BSS only", "Both models" )) {
  ...
  if (exists("estimates_pe") == "FALSE" && exists("estimates_bss") == "FALSE") {
    stop("At least one of 'estimates_pe' or 'estimates_bss' must be supplied.")
  }
  
  # Process PE estimates to common format
  if (exists("estimates_pe") == "TRUE" && params$model_used == "PE only") {
    transformed_pe_data <- process_estimates_pe(analysis_lut, estimates_pe)
  } else {print("PE model not used.")}
  
  # Process BSS estimates to common format
  if (exists("estimates_bss") == "TRUE" && params$model_used == "BSS only") {
    transformed_bss_data <- process_estimates_bss(params, analysis_lut, estimates_bss)
  } else {print("BSS model not used.")}
  
  # Process PE and BSS model estimates to common format
  if (exists("estimates_pe") == "TRUE" && exists("estimates_bss") == "TRUE" && params$model_used == "Both models") {
    transformed_pe_data <- process_estimates_pe(analysis_lut, estimates_pe)
    transformed_bss_data <- process_estimates_bss(params, analysis_lut, estimates_bss)
  } else {print("Only one model transformed as one model output does not exist or params$model_used does not equal `Both models`.")}

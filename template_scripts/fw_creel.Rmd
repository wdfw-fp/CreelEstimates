...
for(ecg in names(inputs_bss)) {
  ...
  ecg_fit <-
    fit_bss(
      ...
      adapt_delta = 0.7,
      max_treedepth = 12,
      threads_per_chain = 4,  # within-chain parallelization via reduce_sum (total CPU = n_cores * threads_per_chain)
      init = "0"
    )
...
# Select and order columns: drop redundant 'median' (keep '50%'), drop 'mad' and 'ess_tail'
overview_combined |>
  select(catch_group, variable, est_cg, mean, sd,
         `2.5%`, `25%`, `50%`, `75%`, `97.5%`,
         rhat, ess_bulk, n_div) |>
  gt(groupname_col = "catch_group") |>
  fmt_number(columns = c(mean, sd, `2.5%`, `25%`, `50%`, `75%`, `97.5%`), decimals = 1) |>
  fmt_number(columns = rhat, decimals = 3) |>
  fmt_number(columns = c(ess_bulk, n_div), decimals = 0) |>
  tab_style(
    style = cell_fill(),
...
map_df(estimates_bss, ~.x$overview) |>
  ...
  tab_style(style = cell_fill(), locations = cells_body(columns = "median"))

...
output:
  html_document:
    ...
    toc_float: yes
    code_folding: hide
    cache.extra: !r list(params)  # Add this
    css: styleRmd2.css
editor_options: 
...
```{r estimates_bss, eval=TRUE, results='hide', fig.show='hide'}
...
for (ecg in names(draws_problems_tables)) {
  ...
  gt_table <- draws_problems_tables[[ecg]] |> 
    ...
    gt::tab_header(
      ...
      subtitle = "Parameters shown meet at least one criteria: R̂ ≥ 1.09 or ESS < 100"
    ) |> 
    gt::tab_style(
    ...
    gt::tab_footnote(
      footnote = "Filtered to 15 rows, sorted by descending R̂",
...
plot_catch_effort <- function(cg) {
  ...
  # calc values for y-axis limit of annotation
  # find y value at the point in the curve closest to the median
  # catch_density <- density(catch_df$C_sum)
  # catch_median_y <- catch_density$y[which.min(abs(catch_density$x - catch_median))]
  ...
  plot_catch <- ggplot(catch_df, aes(x = C_sum)) +
    ...
    # annotate(
    #   "label",
    #    x = catch_quantiles$q[[0.500]],
    #    y = catch_median_y,
    #    label = round(catch_median),
    #    fill = "white",
    #    color = "grey30",
    #    fontface = "bold",
    #    size = 4,
  ...
  # calc values for y-axis limit of annotation
  # find y value at the point in the curve closest to the median
  # effort_density <- density(effort_df$E_sum)
  # effort_median_y <- effort_density$y[which.min(abs(effort_density$x - effort_median))]
...
```{r plot_bss_catch_daily, eval=TRUE}
...
```{r plot_bss_effort_daily, eval=TRUE}
...
```{r plot_bss_cpue_daily, eval=TRUE}
plot_bss_cpue_daily <-
...
### PPC plots
```{r ppc_plots_display, eval=TRUE, fig.height=8, fig.width=10}

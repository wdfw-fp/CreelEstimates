...
date: "`r Sys.Date()`"
params:
  analysis_name: "District 17 North_Humptulips River_fall_salmon_2022"
...
creel$days <- tibble::tibble(
  ...
  DayType = if_else(
    Day %in% params$days_wkend | event_date %in% lu_input$dates_holidays_2015_2030,
    "Weekend", "Weekday"),
...
pe$interview <- dplyr::left_join(
  ...
  dplyr::mutate(
    ...
    angler_type = dplyr::case_when( 
        ...
        boat_used == "Yes" & stringr::str_detect(boat_type, "ontoon|ayak") & fish_from_boat == "Boat" ~ "boat", #allow kayaks as "boat": 
        boat_used == "Yes" & 
          !stringr::str_detect(boat_type, "ontoon|ayak") & 
    ...
    person_count_final = case_when(
        params$person_count_type == "group" ~ total_group_count,
        params$person_count_type == "angler" ~ angler_count
      ),
      
      angler_hours_total = angler_hours * person_count_final
     # changed this to total group count to match census count protocol
  ) |>
  ...
  drop_na(angler_type) #placeholder pending data correction on interviews: 38CE68FD-C974-4736-9F76-091DBC2CE919, 93B6970C-4F81-4375-92D9-C39F935F09A2, CA211A91-CEBC-40C7-9C68-9409A84431B2
...
pe$daily_cpue_catch_est <- pe$interview_and_catch |>
  ...
  summarise(
    ...
    total_hours = sum(angler_hours_total),
...
```{r gg_angler_hours_daily_mean, eval = FALSE}
pe$angler_hours_daily_mean |>
...
```{r gg_angler_hours_est_ribbons, eval = FALSE}
pe$est_effort_s_ts_dt_at |> 
...
```{r pe$proportion_effort_and_interview_hours_by_angler_type, eval=FALSE}

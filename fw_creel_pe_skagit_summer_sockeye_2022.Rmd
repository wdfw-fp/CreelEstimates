...
### Total sockeye estimates by section
...
d2 <- pe$est_catch_s_ts_dt_at |> filter(catch_group %in% sockeye_harvest_groups)
bind_rows(
  d2 |> group_by(section) |>
  summarise(
    catch = sum(est)
...
### Sockeye weekly estimates by catch group
...
### Total sockeye weekly and daily estimates
...
d2 <- pe$est_catch_s_ts_dt_at |> 
  filter(catch_group %in% sockeye_harvest_groups & section == 2) |>
  group_by(section, time_strata, DayType) |> 
  summarise(
    catch_est_mean = sum(catch_est_mean)
  ) |> 
  left_join(creel$days |> 
              filter(open_section_2 == TRUE) |> 
              select(event_date, DayType, time_strata))
...
d3 <- d1 |> 
  ...
  ungroup()
  # write_csv("sockeye_daily_estimates.csv") |> 
  
gt(d3, rowname_col = "rowname") |> 
  gt::fmt_integer(columns = where(is.numeric)) |> 
  gt::tab_options(container.overflow.x = TRUE, container.overflow.y = TRUE)

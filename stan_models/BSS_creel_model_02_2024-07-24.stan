data{
	...
	real<lower=0> O [D,S,G]; //index denoting fishery status by day, section, and gear/angler-type, where 1=open, 0.00001 = closed 
...
transformed parameters{
	...
	for(g in 1:G){
	  for(s in 1:S){
		...
		for(d in 1:D){       
			for(s in 1:S){	
				lambda_C_S[s][d,g] = exp(mu_C[g,s] + to_matrix(omega_C[period[d],],G,S)[g,s]) * O[d,s,g];
				lambda_E_S[s][d,g] = exp(mu_E[g,s] + to_matrix(omega_E[period[d],],G,S)[g,s] + B1 * w[d])* O[d,s,g];
				for(i in 1:H){

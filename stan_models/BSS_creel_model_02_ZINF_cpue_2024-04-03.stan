data{
	...
	int<lower=0> IntA; //total number of angler interviews conducted across all surveys dates where car/trailer angler expansion data (V_A, T_A, A_A) were collected                                         	
...
parameters{
  ...
  real mu_mu_C[G] ; //hyper-prior on mean of mu_C //TB 5/3/2019
	...
	matrix[G,S] eps_mu_C;
	matrix<lower=0,upper=1>[G,S] pzero; //zero-inflation probability for CPUE (probability of an "excess zero")
...
model{
  ...
  for (a in 1:IntC) {
    if (c[a] == 0)
      target += 
        log_sum_exp(bernoulli_lpmf(1 | pzero[gear_IntC[a],section_IntC[a]]), #probability 0 extra zero
        bernoulli_lpmf(0 | pzero[gear_IntC[a],section_IntC[a]]) + neg_binomial_2_lpmf(lambda_C_S[section_IntC[a]][day_IntC[a], gear_IntC[a]] * h[a] , r_C); #probability of 0 but from the neg bim distribution (not extra)
    else
      target += 
        bernoulli_lpmf(0 | pzero[gear_IntC[a],section_IntC[a]]) + neg_binomial_2_lpmf(lambda_C_S[section_IntC[a]][day_IntC[a], gear_IntC[a]] * h[a] , r_C); #probability of nonzero
  }
...
// 	}
// 	//NEED TO ADD POINTWISE LOG LIKELIHOOD FOR Census (tie-in) effort counts - boats
// 	//Census (tie-in) effort counts - anglers
// 	for(e in 1:E_n){
...
// 	}			
// 	//Angler interviews - number of boats
// 	for(a in 1:IntA){
// 		  log_lik[V_n + T_n + A_n + E_n + IntC + IntA + a] = binomial_lpmf(T_A[a]|A_A[a], R_T[gear_IntA[a]]);
// 	}	
// 	//NEED TO ADD IN POINTWISE LOG LIKELIHOOD FOR BOATS PER ANGLER GROUP BINOMIAL...also fix indexing for if 
}

...
if(str_detect(params$index_count_types, "Bank|Boat")) {
  ...
  pe$interview_ang_per_vehic <- bind_rows(
    pe$interview |>
      ...
      summarize(angler_type = "total", anglers_per_vhcl_trlr = sum(person_count_final) / sum(vehicle_count), .groups = "drop")
    ,
    pe$interview |>
      filter(angler_type == "boat") |>
      #could re-introduce here: group_by(section)
      summarize(angler_type = "boat", anglers_per_vhcl_trlr = sum(person_count_final) / sum(trailer_count), .groups = "drop")
    )

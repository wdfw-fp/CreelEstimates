...
creel$interview <- paste0(
  ...
  dplyr::mutate(
    location = if_else(is.na(interview_location), fishing_location, interview_location)  #      dplyr::rename(location = interview_location)
  ) |> 
  dplyr::left_join(lu_input$sections, by = c("location")) |> 
  dplyr::filter(!is.na(section))

creel$catch <- paste0( # this needs to be filtered by the interviews with the analysis's dates / sections immediately, otherwise contains catch across entire project!
  dwg_base$catch,
  "?$where=creel_event_id in('",
  paste(creel$event$creel_event_id, collapse = "','"),
  "')&$limit=100000"
  ...
  inner_join(
    creel$interview |> select(interview_id) # filters catch to only that which corresponds to interviews retained by left_join / filter to create creel$interview
  )

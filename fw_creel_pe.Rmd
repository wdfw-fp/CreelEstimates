...
knitr::opts_chunk$set(warning = FALSE, message = FALSE, fig.width = 9, fig.height = 9)
...
```{r sockeye}
d <- pe$est_catch_s_ts_dt_at |> filter(str_detect(catch_group, "Sockeye_Adult"))
bind_rows(
  ...
  d |> group_by(section, catch_group) |> 
    summarise(angler_type = "total", DayType = "total", across(c(n_obs, est, var, se), ~round(sum(.), 0)), .groups = "drop")
) |> 
...
```{r sockeye totals}
d <- pe$est_catch_s_ts_dt_at |> filter(str_detect(catch_group, "Sockeye"))
bind_rows(
  d |> group_by(section, DayType, angler_type, catch_group) |> 
    summarise(across(c(n_obs, est, var, se), ~round(sum(.), 0)), .groups = "drop"),
  d |> group_by(section, catch_group) |> 

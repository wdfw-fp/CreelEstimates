...
est_pe_prop_effort_sampled_by_angler_type <- function(
    dwg,
    dwg_summ,
    estimates_pe_effort = estimates$pe,
    ...
){
  
  days <- dwg$days
  interview <- dwg$interview
  interview_summ <- dwg_summ$interview
  ...
  interview_summ |>
    ...
    left_join(estimates_pe_effort |> ## join expanded catch estimates in pe$est_effort_s_ts_dt_at to total angler hours from interviews in pe$interview 
    ...
    left_join(
      interview |> distinct(fishing_location, section_num),
      by = "section_num") |> 

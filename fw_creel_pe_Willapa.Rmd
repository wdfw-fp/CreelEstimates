...
date: "`r Sys.Date()`"
params:
  analysis_name: "District 17 South_Naselle River_winter_steelhead_2022"
...
creel$days <- tibble::tibble(
  ...
  Week = 1 + as.numeric(format(event_date, "%U")) - as.numeric(format(as.Date(cut(event_date, "year")), "%U")),
...
pe$effort_census <- dplyr::left_join(
  ...
  dplyr::mutate(
    angler_type = dplyr::case_when(
      ...
      count_type %in% c("Shore - Pontoon/Kick/Kayak", "Shore - Drift/Raft", " Boat - Motor", "Shore - Motor Boat", "Boat - Drift Raft") ~ "boat" #Note "Boats" plural intentionally excluded
    )
  ) |>

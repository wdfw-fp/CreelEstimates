...
# EB realized needed rows duplicated across angler_type for join later in script, so this is my ugly workaround 
...
# code below needs work. DFW creel manual states that days with known effort but no corresponding interview require the analyst to assume a value of CPUE/HPUE for that day. This is a rough attempt at that using values from point_estimate_data_prelim$cpue_summary

# EB Using a rows upsert approach to stuff in CPUE for days with known effort but no interview. This spirals out of control with additional catch groups and should probably be done using a for loop or apply framework 
...
test <- estimates$daily_effort_cpue_catch |>
  group_by(section, angler_type, Month, DayType, n_days, N_days) |> 
  summarize(
    n = n(),
    Sum_daily_effort_sampled_days = sum(mean_daily_TI_Expan),
    Mean_daily_effort = Sum_daily_effort_sampled_days / n,
    Mean_daily_effort_2 = mean(mean_daily_TI_Expan),
    Variance_daily_effort = var(mean_daily_TI_Expan),
    Total_effort = Mean_daily_effort * N_days,
    Variance_total_daily_effort = (N_days^2) * (Variance_daily_effort / n_days) * (1-(n_days/N_days))
  ) |> 
...
```{r, include = FALSE}
# all_catch_total <- estimates$monthly_catch_total |> 
#   group_by(catch_group) |> 
#   summarise(total_catch = sum(total_catch))
# 
# all_catch_total_table <- estimates$monthly_catch_total |> 
#   group_by(catch_group) |> 
#   summarise(total_catch = sum(total_catch)) |> 
#   kbl(caption = "Total catch estimates for coho and spring Chinook grouped by origin, life stage, and fate during the 2021 Skagit River fall salmon fishery from 08/14/2021 - 12/31/2021.",digits =2) |> 
#   # kable_classic(full_width = F, html_font = "Cambria") |> 
#   kable_styling(bootstrap_options = c("striped", "hover"))
# 
# print(all_catch_total)
# 

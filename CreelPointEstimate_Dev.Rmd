...
This file uses a workflow developed by the DFW creel dev crew to summarize and generate creel estimates (angler effort, catch, and catch per unit effort (cpue)) from freshwater monitoring data housed on data.wa.gov, using deterministic, maximum likelihood estimators (see Kale Bentley's original creel analysis files, Pollock et al. 1994).
...
d_days <- rows_upsert(d_days,
    bind_rows(
      # tibble(section = "1", closure_begin = "2017-03-17", closure_end = "2017-03-17"), # salmon opens on 9/1 for sections 3,4
...
point_estimate_data_prelim$interview_daily_totals_total <- point_estimate_data_prelim$interview |>
  ...
  group_by(section, time_strata, DayType, event_date, day_index, catch_group) |> 
  summarise(
    angler_hours_total_dailysum = sum(angler_hours_total),
    catch_dailysum = sum(fish_count), .groups = "drop",
...
# CPUE for bank effort 
point_estimate_data_prelim$interview_daily_totals_bank <- point_estimate_data_prelim$interview |>
...
point_estimate_data_prelim$interview_daily_totals <- point_estimate_data_prelim$interview_daily_totals_total |> 
  ...
  bind_rows(point_estimate_data_prelim$interview_daily_totals_bank)
...
point_estimate_data_prelim$interview_daily_totals_complete <- point_estimate_data_prelim$interview_daily_totals |>
  select(section, event_date, angler_type, catch_group, catch_dailysum, cpue_rom) |> 
  ungroup() |>
...
# calculate mean cpue values by DayType, time_strata, Catch_Group to use for assumed values and complete the dataframe with implicit missing values of summarized CPUE (These should all essentially be 0)
...
point_estimate_data_prelim$cpue_summary <- point_estimate_data_prelim$interview_daily_totals |> 
  ...
  summarise(
    mean_strata_cpue = mean(cpue_rom) # EB using ratio of the means for now but should circle back to this soon
  ) |> 
  ungroup() |>
  complete(section, time_strata, DayType, angler_type, catch_group,
           fill = list(mean_strata_cpue = 0))
...
point_estimate_data_prelim$daily_CPUE <- point_estimate_data_prelim$interview_daily_totals_complete |> 
  left_join(point_estimate_data_prelim$cpue_summary, by = c("section", "angler_type", "DayType", "catch_group"))|> 
  select(section, time_strata, event_date, angler_type, catch_group, catch_dailysum, cpue_rom, mean_strata_cpue)
...
estimates$daily_effort_cpue_catch <- point_estimate_data_prelim$effort_interviews_final |> 
  ...
  mutate(
    catch_estimate = if_else(!is.na(cpue_rom), mean_daily_TI_Expan * cpue_rom, mean_daily_TI_Expan * mean_strata_cpue)
    ) |> 
...
estimates$time_strata_effort_total <- estimates$time_strata_effort_by_daytype |>
  ungroup() |>
  left_join(point_estimate_data_prelim$degrees_freedom, by = c("section", "angler_type")) |> # add degrees of freedom per angler_type and section for time_strata

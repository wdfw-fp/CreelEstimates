...
days_join <- d_days |> 
  ...
  mutate(at_1 = "total", at_2 = "boat", at_3 = "bank"
  ) |> 
...
# calculate mean cpue values by DayType, Month, Catch_Group to use for assumed values and complete the dataframe with implicit missing values of summarized CPUE (These should all essentially be 0)
...
point_estimate_data_prelim$cpue_summary <- point_estimate_data_prelim$interview_daily_totals |> 
  ...
  ungroup() |>
  complete(section, Month, DayType, angler_type, catch_group,
...
point_estimate_data_prelim$daily_CPUE <- point_estimate_data_prelim$interview_daily_totals_complete |> 
  left_join(point_estimate_data_prelim$cpue_summary, by = c("section", "Month", "angler_type", "DayType", "catch_group"))|> 
  select(section, event_date, angler_type, catch_group, catch_dailysum, cpue_rom, mean_strata_cpue)
...
estimates$daily_effort_cpue_catch |>
  filter(section == 3, angler_type == "total") |> 
  ggplot(aes(x = event_date, y = catch_estimate)) +
  geom_point() +
  geom_smooth() +

...
catch_check <- creel$catch |> 
  group_by(catch_group) |> 
  summarize(
    count_sum = sum(fish_count)
  )
...
sum(point_estimate_data_prelim$catch_data_pivot_wide$Steelhead_Adult_UM_Released)
...
sum(point_estimate_data_prelim$interview_and_catch$fish_count)
...
view(point_estimate_data_prelim$angler_hours_summary)
...
# pooled CPUE tabled on 4/7/2022, circle back at some point 

# CPUE for bank and boat effort 
# cpuecheck <- point_estimate_data_prelim$interview_and_catch |>
#    mutate(
#     cpue_interview = fish_count / angler_hours_total
#   ) |>
#   group_by(section, DayType, event_date, day_index, time_strata, catch_group, angler_type) |>
...
# before <- point_estimate_data_prelim$interview_daily_totals |>
#   ungroup() |> 
#   select(section, event_date, angler_type, catch_group, catch_dailysum, cpue_rom, cpue_mor) |> 
#   filter(catch_group == "Steelhead_Adult_UM_Released", section == 1, angler_type == "bank")
# 
# after <- point_estimate_data_prelim$interview_daily_totals_complete <- point_estimate_data_prelim$interview_daily_totals |>
#   ungroup() |> 
#   select(section, event_date, angler_type, catch_group, catch_dailysum, cpue_rom, cpue_mor) |> 
...
point_estimate_data_prelim$interview_daily_totals_complete <- point_estimate_data_prelim$interview_daily_totals |>
  ...
  filter(catch_group == "Steelhead_Adult_UM_Released") |> # placeholder spot to filter catch groups
  complete(section, event_date, angler_type, fill = list(catch_dailysum = 0)) |> 
  filter(!(is.na(angler_type))) |> 
  mutate(catch_group = "Steelhead_Adult_UM_Released") |> 
  # distinct() |>
...
#   ) |> 
#   filter(mean_strata_cpue_mor > 0)
# 
# after <- point_estimate_data_prelim$interview_daily_totals |>
#   group_by(section, time_strata, DayType, angler_type, catch_group) |>
#   summarise(
#     mean_strata_cpue_mor = mean(cpue_mor), # Fisheries Techniques (Jones and Pollock 2012) suggest MOR for roving survey design
#     mean_strata_cpue_rom = mean(cpue_rom)
...
# view(point_estimate_data_prelim$cpue_summary)
...
# SH_check <- point_estimate_data_prelim$daily_CPUE |> 
#   filter(catch_group == "Steelhead_Adult_UM_Released" & angler_type %in% c("bank", "boat")) |> 
#   summarize(
#     raw_catch = sum(catch_dailysum)
#   )

# losing 1 steelhead relative to raw catch data.. need to track down why, probably lost when joining interview to catch tables
...
# EB 4/6/2022 - revisit this to keep assessing if this is correct 

# estimates$daily_effort_cpue_catch <- point_estimate_data_prelim$effort_interviews_final |>
#   left_join(point_estimate_data_prelim$daily_CPUE, by = c("section", "time_strata", "event_date", "angler_type")) |>
#     filter(angler_type %in% c("bank","boat")) |>
#   mutate(
#     catch_estimate_mor = if_else(!is.na(cpue_mor), mean_daily_TI_expan * cpue_mor, mean_daily_TI_expan * mean_strata_cpue_mor)
#     # catch_estimate_rom = if_else(!is.na(cpue_rom), mean_daily_TI_expan * cpue_rom, mean_daily_TI_expan * mean_strata_cpue_rom),
...
view(point_estimate_data_prelim$effort_summary)


effort_join <- point_estimate_data_prelim$effort_interviews_final |> 
  filter(!is.na(DayType)) |> 
...
estimates$daily_effort_cpue_catch <- point_estimate_data_prelim$daily_CPUE  |>
  # left_join(point_estimate_data_prelim$effort_interviews_final, by = c("section", "time_strata", "event_date", "angler_type")) |>
   left_join(effort_join) |>
...
weekly_test <- estimates$daily_effort_cpue_catch |> 
  group_by(time_strata, angler_type) |> 
  summarize(
    total_catch = sum(catch_estimate),

...
d_days <- rows_upsert(d_days,
    bind_rows(
      tibble(section = "1", closure_begin = "2017-01-31", closure_end = "2017-01-31") # placeholder closure date; need to supply at least one date here because we use open / closed dates to determine times_strata estimation periods (field n_days further down in code)
      # ,
...
# index counts that are direct counts of bank anglers and boat anglers
...
#     # daily_sum_index_count_from_interview = "", # filler to match formatting of vehicle / trailer chunk below 
#     angler_hours_total = sum(angler_hours_total),
#     # anglers_per_index_count_from_interview = "")
  
      }else{
      if(str_detect(index_count_types, c("Vehicle Only", "Trailers Only"))){ # index counts of vehicles and trailers
...
point_estimate_data_prelim$census_counts_all <- point_estimate_data_prelim$census_counts_both_types |> 
  ...
  summarise(
    TI_expan_mean = mean(TI_expan),
    TI_expan_total = sum(census_count) / sum(index_count)) |> 
  right_join(lut$census_expansion, by = c("angler_type", "section")) |>
  ...
  mutate(
    ...
    TI_expan_total = replace_na(TI_expan_total, 1),
    TI_expan_direct = TI_expan_total / p_TI) |> 
  mutate(
    TI_expan_final = if_else(census_expansion_method == "direct", TI_expan_direct, TI_expan_indirect)
  ) 
...
estimates$time_strata_catch_total |>
  filter(catch_group %in% c("Steelhead_Adult_AD_Kept", "Steelhead_Adult_UM_Released")) |>

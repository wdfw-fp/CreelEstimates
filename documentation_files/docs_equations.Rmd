...
The following equations document the process to generate angler effort and catch estimates for stratum $k$, a variable used to represent estimate strata for time-period, day-type, and section $s$ when these strata are not specified in an equation. Angler-type stratum $g$ are specified to help clarify the conversion of counts of non-angler objects (e.g., vehicles, boat trailers) to angler count estimates. Angler trips $i$ are a unit of data collection corresponding to a completed interview of angler type $g$, categorized as either as a bank angler or boat angler.
...
$$
\hat{R}_{k,d} = \frac {\sum_{i = 1}^{t_d}{{c_{d,i}}}} {\sum_{i = 1}^{t_d}{{h_{d,i}}}}
$$

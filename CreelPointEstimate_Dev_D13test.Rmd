...
params:
  ...
  water_body: "Tokul Creek"
  date_start: "2021-12-01"   
  date_end: "2022-02-15"
  time_strata: "month"
  sections: "lut_water_body_location_d13_tokul_test.csv"
output:
  html_document:
    fig_caption: yes
    theme: default
...
This file uses a workflow developed by the DFW creel dev crew to summarize and generate creel estimates (angler effort, catch, and catch per unit effort (cpue)) from freshwater monitoring data housed on data.wa.gov, using deterministic, maximum likelihood estimators (see Kale Bentley's original creel analyis files, Pollock et al. 1994).
...
lut <- map(
  list(
    river_loc = "input_files/lut_13_River.Locations_2021-10-21_Tokul_test.csv",
...
# Tokul Creek closed to fishing at 17:00 - maybe modify this to accommodate unique times at which fishery opens / closes
# this uses dawn and dusk to define DayL, may need to modify to align with specific legal fishing hours
...
d_days <- rows_upsert(d_days,
    bind_rows(
      tibble(section = "1", closure_begin = "2021-12-27", closure_end = "2021-12-28"), # example out days for winter weather
      tibble(section = "1", closure_begin = "2022-01-02", closure_end = "2022-01-05") # example out days for winter weather
    ) |>
      rowwise() |>
...
```{r point_estimate_data_prelim$effort_counts_and_interviews, include = FALSE}
...
point_estimate_data_prelim$effort_interviews_totalandboat <- point_estimate_data_prelim$effort_counts_and_interviews |>
  ungroup() |> 
  left_join(point_estimate_data_prelim$census_counts_all, by = c("section", "angler_type")) |> 
  select(-c(Month, Week)) |> 
  mutate(
...
point_estimate_data_prelim$effort_interviews_bank_1 <- point_estimate_data_prelim$effort_interviews_totalandboat |>
  ...
  # mutate(
  #   mean_daily_TI_Expan_bank = total - boat # this breaks on Tokul Creek data, using only total
  # )
  mutate(
    mean_daily_TI_Expan_bank = total
  )
...
point_estimate_data_prelim$effort_interviews_bank_2 <- point_estimate_data_prelim$effort_interviews_totalandboat |> 
  ...
  select(-c(mean_daily_TI_Expan, total)) |> # removed boat from Tokul Creek example
  rename(
...
point_estimate_data_prelim$total_days <- point_estimate_data_prelim$effort_interviews_final |>
  ...
  count(section, DayType, Month, angler_type, name = "N_days")
...
point_estimate_data_prelim$effort_interviews_final <- point_estimate_data_prelim$effort_interviews_final |> 
  left_join(point_estimate_data_prelim$total_days, by = c("section", "DayType", "Month", "angler_type")) |> 
  left_join(point_estimate_data_prelim$sampled_days, by = c("section", "DayType", "Month", "angler_type")) |>
...
# EB code below needs work. DFW creel manual states that days with known effort but no corresponding interview require the analyst to assume a value of CPUE/HPUE for that day. This is a rough attempt at that using values from point_estimate_data_prelim$cpue_summary
# Using a rows upsert approach to add in CPUE for days with known effort but no interview. This spirals out of control with additional catch groups and should probably be done using a for loop or apply framework.. if really necessary. Could be topic of discussion with creel dev crew
...
```{r estimates$monthly_effort_total, message = FALSE, warning = FALSE,results = "asis",include=TRUE, echo=FALSE}
...
monthly_effort_total_table <- estimates$monthly_effort_total |>
...
print(monthly_effort_total_table)

estimates$monthly_effort_total |> filter(angler_type == "total") |> group_by(section) |> summarize(total_effort = sum(total_effort))
```

```{r, include=TRUE, fig.align="center",fig.width= 14, fig.height= 8, fig.cap=c("Figure 1. Monthly effort estimates (angler hours) during the 2021 Skagit River fall salmon fishery from 08/14/2021 - 12/31/2021. Error bars show 95 percent confidence intervals."),echo=FALSE,warning = FALSE}
options(scipen=10000)

estimates$monthly_effort_total |>
  # arrange(section) |> 
  # mutate(name=factor(section), levels=section) |> 
  # filter(!grepl(c("Steelhead"),Catch_Group)) |> 
  ggplot(aes(x = Month, y = total_effort, fill = angler_type)) +
...
```{r estimates$monthly_catch_total, message = FALSE, warning = FALSE,results = "asis",include=TRUE, echo=FALSE}
...
monthly_catch_total_table <- estimates$monthly_catch_total |>
  filter(angler_type == "total") |> 
  kbl(caption = "Monthly catch statistics for coho and Chinook during the 2021 Skagit River fall salmon fishery from 08/14/2021 - 12/31/2021, combing both bank and boat angler types.",digits =2) |> 
  # kable_classic(full_width = F, html_font = "Cambria") |> 
  kable_styling(bootstrap_options = c("striped", "hover"))

print(monthly_catch_total_table)

```


```{r, include=TRUE, fig.align="center",fig.width= 14, fig.height= 8, fig.cap=c("Figure 1. Monthly harvest estimates for ad-clipped and unmarked coho during the 2021 Skagit River fall salmon fishery from 08/14/2021 - 12/31/2021. Error bars show 95 percent confidence intervals."),echo=FALSE,warning = FALSE}

estimates$monthly_catch_total |> 
  # filter(!grepl(c("Steelhead"),Catch_Group)) |> 
  filter(angler_type == "total" & catch_group %in% c("Coho_Adult_UM_Kept", "Coho_Adult_AD_Kept")) |>
  ggplot(aes(x = Month, y = total_catch)) +
  geom_bar(position = "dodge", stat = "identity", color = "black") +
  ...
  theme(legend.text=element_text(size=8)) +
  theme_bw(base_size = 12) + 
  facet_wrap(. ~ catch_group) # removed section for Tokul Creek test
...
# Adding together monthly effort estimates and associated variance and calculating SE, CV, and 95% CI's
estimates$total_effort <- estimates$monthly_effort_total |>
...
# Adding together monthly catch estimates and associated variance and calculating SE, CV, and 95% CI's

estimates$total_catch <- estimates$monthly_catch_total |>
  left_join(point_estimate_data_prelim$degrees_freedom_total, by = c("section", "angler_type")) |> 
